...
const shouldDropEvent = (playerState: PlayerStatePayload) => {
  return playerState.context_uri === undefined || playerState.context_uri === null;
};
...
class PlayerStore {
  ...
  constructor(rootStore: RootStore, interappActions: InterappActions, socket: Socket) {
  ...
  get isDj() {
    return isDjUri(this.state.context_uri);
  }
  ...
  get isDjTalking() {
    return (
      ...
      this.currentTrack.album.name === 'DJ' &&
      this.currentTrack.artist &&
      this.currentTrack.artist.name === 'DJ'
    );
  }
  ...
  get contextUri(): string {
    if (isSearchURI(this.state.context_uri) || this.rootStore.queueStore?.isCurrentProviderQueue) {
  ...
  playUriFromContext(contextUri: string, uri: string, featureIdentifier: string, interactionId: string, uid?: string) {
    if (this.useSuperbirdEndpoints) {
      this.interappActions.playUri(contextUri, featureIdentifier, interactionId, uri, uid);
    } else {
      this.interappActions._playUriFromContext(contextUri, uri, featureIdentifier);
    }
  }
  ...
  seek(position: number) {
    if (this.canSeek && this.currently_seeking === undefined) {
      ...
      this.currently_seeking = window.setTimeout(this.resetCurrentlySeeking, 2000);
    }
  }
  ...
  summonDj(feature: string) {
    if (this.isDj) {
      this.interappActions.summonDj();
    } else {
      this.playItem(DJ_URI, feature);
    }
  }
  ...
  onDjTalkingChange(callback: () => void) {
    reaction(
      () => this.isDjTalking,
      () => callback(),
    );
