...
const INITIAL_VIEW_STACK: ViewStackItem[] = [{ view: View.CONTENT_SHELF }, { view: View.NPV }];
...
class ViewStore {
  ...
  backSkippingQueue() {
    const listIndex = this.viewStack.findIndex(viewStackItem => viewStackItem.view === View.QUEUE);
    ...
    if (isSupportedUriType(playerStore.contextUri)) {
      switch (this.currentView) {
        ...
        case View.CONTENT_SHELF:
          this.showTracklist();
          setTimeout(() => shelfStore.shelfController.goBackToHome(), transitionDurationMs);
          break;
        case View.NPV:
  ...
  maybeDoBackSideEffect(popedViewStackItem?: ViewStackItem) {
    ...
    if (
      ...
      this.rootStore.tracklistStore.tracklistUiState.initializeTracklist(this.rootStore.playerStore.currentContextItem);
    } else if (
      ...
      this.currentViewStackItem.state.value.uri !== tracklistStore.tracklistUiState.contextUri
    ) {
      tracklistStore.tracklistUiState.handleSupportedTracklists(this.currentViewStackItem.state.value);
