...
export type IphonePhoneCallState = {
  ...
  remote_id: string;
  display_name: string;
  status: 'Disconnected' | 'Sending' | 'Ringing' | 'Connecting' | 'Active' | 'Held' | 'Disconnecting';
...
class IphonePhoneCallStore implements PhoneCallStore {
  ...
  get callerName(): string | undefined {
    return hasName(this.callerNumber, this.callState?.display_name) ? this.callState?.display_name : undefined;
  }
  ...
  get isRingingIncoming(): boolean {
    return this.callState?.call_dir === 'Incoming' && this.callState?.status === 'Ringing';
  }

  get isRingingOutgoing(): boolean {
    return this.callState?.call_dir === 'Outgoing' && this.callState?.status === 'Sending';
  }
