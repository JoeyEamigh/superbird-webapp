...
export const isAllowedUriType = (item: { uri: string }) =>
  item && item.uri !== 'spotify:delimiter' && !item.uri.startsWith('spotify:meta:');

export const shouldDropQueueEvent = ({ payload: { next, current } }: QueueEvent): boolean => {
  const noRealQueueItems = !next.filter(item => isAllowedUriType(item)).length;
...
export const decorateQueueItem = action(
  (queueEventItem: QueueEventItem, index: number, position: POSITION): QueueItem =>
    Object.assign(queueEventItem, {
      identifier: `${queueEventItem.uid || queueEventItem.uri}-${position}`,
      queue_index: index,
      artist_name: getArtistNames(queueEventItem.artist_name, queueEventItem.artists),
    }),
);
...
class QueueStore {
  ...
  constructor(
    ...
    playerStore.onDjTalkingChange(() => {
      if (!this.playerStore.isDjTalking && this._stashed) {
        // fake a queue update with the stashed event
        this.onMiddlewareEvent(this._stashed);
        this._stashed = undefined;
      }
    });
  }
  ...
  get previous(): QueueItem[] {
    return this._previous
      ...
      .map((item, index, items) => decorateQueueItem(item, -1 * (items.length - index), POSITION.PREVIOUS));
  }
  ...
  stashQueueEvent(event: QueueEvent): boolean {
    if (this.playerStore.isDjTalking) {
      this._stashed = event;
      return true;
    }
    return false;
  ...
  onMiddlewareEvent(msg: QueueEvent): void {
    switch (msg.type) {
      case 'com.spotify.play_queue':
        if (!this.stashQueueEvent(msg) && !shouldDropQueueEvent(msg)) {
