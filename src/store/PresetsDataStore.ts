...
import { autorun, makeAutoObservable, ObservableMap, reaction, runInAction } from 'mobx';
...
const _isValidResponse = (response: any): boolean => response && response.result && Array.isArray(response.result);
...
const isValidSetPresetResponse = (response: any): boolean =>
  response === null || (response && response.result && Array.isArray(response.result));
...
class PresetsDataStore {
  ...
  presets: ObservableMap<PresetNumber, Preset> = new ObservableMap<PresetNumber, Preset>();
  ...
  async loadPresets() {
    ...
    const { result, success } = await tryActionNTimes({
      asyncAction: () => {
        return this.remoteConfigStore.graphQLPresetsEnabled ? this.fetchPresets() : this._fetchPresets();
      },
  ...
  private async fetchPresets(): Promise<GetPresetsResponse> {
    ...
    try {
      this.loading = true;
      const response = await this.interappActions.getPresets(this.versionStatusStore.serial);
  ...
  private async callSetPreset(
    ...
    try {
      const response = await this.interappActions.setPreset(getSetPresetRequest(contextUri, presetNumber, source));
  ...
  async savePreset(contextUri: string, presetNumber: PresetNumber, source: PresetSavingSource): Promise<boolean> {
    ...
    runInAction(() => {
      ...
      if (savePresetResponse.result) {
        savePresetResponse.result.forEach(preset => this.presets.set(preset.slot_index, preset));
      } else {
        this.loadPresets();
      }
    });
