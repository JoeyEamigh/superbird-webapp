...
type RemoteControlMessage = RemoteControlConnectionStatus | TransportConnectionStatus;
...
class RemoteControlStore {
  ...
  isConnectedPhone(address: string | undefined): boolean {
    ...
    const isCurrentDevice =
      address === bluetoothStore.currentDevice?.address && this.macAddress === bluetoothStore.currentDevice?.address;
  ...
  isNewPhoneConnected(): boolean {
    ...
    return (
      remoteControlStore.macAddress === bluetoothStore.currentDevice?.address && remoteControlStore.interappConnected
    );
  }
