...
import { isValidPresetNumber, parsePresetNumber, PresetNumber } from 'store/PresetsDataStore';
...
const TIMEOUT_ERROR_MESSAGE = 'I didn’t understand what you said. Tap to try again.';
...
class VoiceStore {
  ...
  constructor(rootStore: RootStore, socket: Socket, middlewareActions: MiddlewareActions) {
    ...
    switch (msg.type) {
      ...
      case 'voice_error':
        if (msg.payload.domain === 'OFFLINE') {
          this.onError('offline', 'Looks like you’re offline.\nMake sure cellular data is turned on.');
        } else {
  ...
  onMicrophoneLevel(msg: MicrophoneLevelMessage): void {
    ...
    if (this.microphoneLevelsSlidingWindow.length) {
      this.micLevelMovingAverage =
        this.microphoneLevelsSlidingWindow.reduce((sum, v) => sum + v, 0) / this.microphoneLevelsSlidingWindow.length;
    }
  }
  ...
  closeVoiceOverlayAndDoEffectAfterTimeout(effect: () => void, timeout: number): void {
    const { overlayController } = this.rootStore;
  ...
  showVoiceConfirmation(): void {
    ...
    this.closeTimeoutId = window.setTimeout(
      mobxAction(() => {
        ...
        this.closeVoiceOverlayAndDoEffectAfterTimeout(() => {
          setTimeout(() => this.resetVoiceSessionState(), OVERLAY_TRANSITION_DURATION_MS);
  ...
  goToContentShelfCategory(goToContentId: string) {
    ...
    this.closeVoiceOverlayAndDoEffectAfterTimeout(async () => {
      ...
      shelfStore.shelfController.handleShelfTitleSelected(parseCategoryId(goToContentId));
  ...
  goToVoiceResult(voiceIntent: string, contentId: string) {
    ...
    const timeToNpv = voiceIntent === SHOW_INTENT ? DEFAULT_TIMEOUT_TO_NPV : PLAY_TIMEOUT_TO_NPV;
    ...
    this.closeVoiceOverlayAndDoEffectAfterTimeout(() => {
      viewStore.backToContentShelf(timeToNpv + MINIMUM_THINKING_TIME_FOR_SEARCH_RESULT);
  ...
  summonDj() {
    ...
    this.closeVoiceOverlayAndDoEffectAfterTimeout(() => {
      playerStore.summonDj(FEATURE_IDENTIFIER_VOICE);
      viewStore.showNpv();
    }, MINIMUM_THINKING_TIME_FOR_SEARCH_RESULT);
  }
  ...
  handleShowTracklistUiAction(nluMessage: NluMessage) {
    const { tracklistStore, errorHandler, viewStore, shelfStore } = this.rootStore;
    ...
    if (contentId) {
      ...
      errorHandler.logUnexpectedError(new Error('Should show search result but received no content_id'));
    }
  ...
  onVoiceIntent(nluMessage: NluMessage): void {
    ...
    switch (voiceIntentAction) {
      ...
      case UiAction.SHOW_TRACKLIST:
        setTrackViewFeatureIdentifier(
          actionIntent === PLAY_PODCAST_ACTION ? FEATURE_IDENTIFIER_VOICE : FEATURE_IDENTIFIER_SHOW_ME,
        );
      ...
      case UiAction.GO_TO_CONTENT:
        if (goToContentId && shelfStore.shelfController.getCategoryFromId(parseCategoryId(goToContentId))) {
          this.goToContentShelfCategory(goToContentId);
        } else {
          errorHandler.logUnexpectedError(
            new Error(`Should go to specific category but no matching shelf category found: ${goToContentId}`),
          );
        }
        break;
      case UiAction.GO_TO_LIBRARY:
        if (shelfStore.shelfController.firstLibraryCategoryId) {
          this.goToContentShelfCategory(shelfStore.shelfController.firstLibraryCategoryId);
        }
        break;
      ...
      case UiAction.SAVE_TO_PRESET:
        if (slots?.preset?.length && isValidPresetNumber(slots.preset[0])) {
          presetsController.presetsUiState.setTempPreset(parsePresetNumber(slots.preset[0]));
          presetsDataStore.savePreset(playerStore.contextUri, parsePresetNumber(slots.preset[0]), 'voice');
          this.showVoiceConfirmation();
        } else {
          if (presetsDataStore.emptyPresetNumbers.length === 0) {
            ...
            presetsController.presetsUiState.setTempPreset(presetsDataStore.emptyPresetNumbers[0]);
            presetsDataStore.savePreset(playerStore.contextUri, presetsDataStore.emptyPresetNumbers[0], 'voice');
            this.showVoiceConfirmation();
          }
        }
        break;
      ...
      case UiAction.UNKNOWN:
        errorHandler.logUnexpectedError(new Error(`Unknown voice intent: ${intent} Action: ${actionIntent}`));
        overlayController.resetAndMaybeShowAModal();
        break;
      default:
        this.onError(nluMessage.custom.error);
  ...
  closeListeningAfterTimeout(): void {
    if (!this.rootStore.viewStore.isOnboarding) {
      window.clearTimeout(this.closeTimeoutId);
      this.closeTimeoutId = window.setTimeout(() => this.cancel(), TIMEOUT_BEFORE_CLOSING_LISTENING_MS);
    }
  }
  ...
  intendedAction(voiceMessage: NluMessage): UiAction {
    ...
    const { intent, action, go_to_content_id: goToContentId, error, slots, query } = voiceMessage.custom;

    if (action?.startsWith('SHOW_MY_') && goToContentId && !goToContentId.includes(VOICE_IDENTIFIER)) {
      return UiAction.GO_TO_CONTENT;
    } else if (action?.startsWith('SHOW_MY_') && goToContentId?.includes(VOICE_IDENTIFIER)) {
      ...
      action &&
      [SAVE_TO_COLLECTION_ACTION, SAVE_TO_COLLECTION_PODCAST_ACTION].includes(action) &&
    ...
    } else if (intent === SHOW_INTENT && action === 'UNKNOWN' && query === 'DJ') {
      return UiAction.SUMMON_DJ;
    } else if (NO_CONFIRMATION_INTENTS.includes(intent) || action === PLAY_SPOTIFY_ACTION) {
    ...
    } else if (SEARCH_RESULT_INTENTS.includes(intent) && !error && !goToContentId && voiceMessage.body?.length) {
      return UiAction.SHOW_SEARCH_RESULT;
    ...
    } else if (intent === NO_INTENT && !voiceMessage.body?.length && !goToContentId) {
      return UiAction.ERROR_RESULT;
    } else if (SEARCH_RESULT_INTENTS.includes(intent) && voiceMessage.body?.length === 0) {
      errorHandler.logUnexpectedError(new Error(`Voice result intent without items: ${intent}`));
      return UiAction.NO_CONFIRMATION;
    }
