...
class PhoneConnectionStore {
  ...
  constructor(rootStore: RootStore, middlewareActions: MiddlewareActions) {
    ...
    this.phoneConnectionContextMenuUiState = new PhoneConnectionContextMenuUiState(
      this,
    ...
    rootStore.bluetoothStore.onBluetoothDeviceListUpdates(() => this.handleDeviceListUpdate());

    rootStore.bluetoothStore.onCurrentDeviceUpdates(() => {
      this.handleCurrentDeviceUpdate();
    });

    rootStore.remoteControlStore.onRemoteControlMacAddress(() => this.handlePhoneConnectionUpdate());

    rootStore.bluetoothStore.onPairingStarted(() => this.handleBluetoothPairing());
  ...
  handleAddNewPhoneClick = () => {
    this.rootStore.ubiLogger.settingsUbiLogger.logPhoneConnectionNewPhoneClick(this.newPhoneItemPosition);
    this.showAddNewPhone();
  };

  handleAddNewPhoneDialPress = () => {
    this.rootStore.ubiLogger.settingsUbiLogger.logPhoneConnectionNewPhoneDialPressed(this.newPhoneItemPosition);
    this.showAddNewPhone();
  };
  ...
  handleDeviceListUpdate() {
    ...
    if (!!this.phoneToConnectOrForget && !phoneInList) {
      ...
      this.setPhoneConnectionModal(PhoneConnectionModalView.FORGET_PHONE_SUCCESS);
  ...
  handleCurrentDeviceUpdate() {
    ...
    const selectConnectionFailed =
      this.isSelectPhoneProgress && this.phoneToConnectOrForget?.address !== bluetoothStore.currentDevice?.address;
    const connectNewFailed =
      !!this.phoneToAdd && this.isNewPairingModal && this.phoneToAdd?.address !== bluetoothStore.currentDevice?.address;
  ...
  handlePhoneForgetNotSuccessful() {
    ...
    if (this.isPhoneForgetProgressModal) {
      this.setPhoneConnectionModal(PhoneConnectionModalView.FORGET_PHONE_FAILURE);
      window.setTimeout(() => {
        this.resetPhoneConnectionFlow();
      }, 5_000);
    }
  ...
  submitForget(device: Device) {
    const { bluetoothStore } = this.rootStore;
    this.phoneConnectionContextMenuUiState.dismissMenu();
    this.setPhoneConnectionModal(PhoneConnectionModalView.FORGET_PHONE_PROGRESS);
    bluetoothStore.forget(device.address);
    this.phoneForgetTimeoutId = window.setTimeout(() => this.handlePhoneForgetNotSuccessful(), 60_000);
  }
  ...
  private submitSelectPhone(device: Device) {
    ...
    const isConnectedPhone = remoteControlStore.isConnectedPhone(device.address);
    if (!isConnectedPhone) {
      this.setPhoneConnectionModal(PhoneConnectionModalView.SELECT_PHONE_PROGRESS);
      this.setPhoneToConnectOrForget(device);
      bluetoothStore.select(device.address);
      this.phoneConnectionTimeoutId = window.setTimeout(() => this.handlePhoneConnectionNotSuccessful(), 75_000);
    }
  }
  ...
  private logAddNewPhone() {
    ...
    this.rootStore.ubiLogger.settingsUbiLogger.logPhoneConnectionAddNewPhoneDialog(position);
  }
  ...
  get isPhoneForgetProgressModal() {
    return this.phoneConnectionModal === PhoneConnectionModalView.FORGET_PHONE_PROGRESS;
  }

  get isNewPairingModal(): boolean {
    return this.phoneConnectionModal === PhoneConnectionModalView.ADD_NEW_PAIRING;
  }

  get isSelectPhoneProgress(): boolean {
    return (
      !!this.phoneToConnectOrForget && this.phoneConnectionModal === PhoneConnectionModalView.SELECT_PHONE_PROGRESS
    );
  }

  handlePhoneForgetConfirmClick = () => {
    if (this.phoneToConnectOrForget) {
      this.rootStore.ubiLogger.settingsUbiLogger.logForgetConfirmClick(this.phoneToConnectOrForget.name);
  ...
  getPhoneConnectionDisplayStatus(device?: Device): string {
    ...
    const isCurrentDevice = device?.address === bluetoothStore.currentDevice?.address;
