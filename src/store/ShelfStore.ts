...
import { ChildrenOfItemResponseItem, isChildrenOfItemType } from 'store/ChildItemStore';
import {
  HomeChildItem,
  HomeItem,
...
export const FIRST_PAGE_SIZE = INITIAL_VISIBLE_ITEMS + NUMBER_OF_ITEMS_TO_SHOW_ON_MORE;
...
export const getCategoryItems = (category: Category, handleVoice = false): ShelfItem[] => {
  ...
  if (category.parsedId !== VOICE_IDENTIFIER && firstItem.type === ItemType.UNPLAYABLE_ITEM) {
    return [
      firstItem,
      {
        ...firstItem,
  ...
  if (category.parsedId === VOICE_IDENTIFIER) {
    if (handleVoice && firstItem.type === ItemType.UNPLAYABLE_ITEM && secondItem?.type !== ItemType.PLAYABLE_ITEM) {
      return [
        firstItem,
        {
          ...firstItem,
...
const isPlaceHolderUri = (uri: string): boolean => uri.includes('0000000000000000000000');

const getShelfItem = (item: ChildrenOfItemResponseItem | HomeChildItem, itemCategory: string): ShelfItem => {
...
class ShelfStore {
  ...
  async getShelfData(): Promise<void> {
    ...
    try {
      ...
      this.loading = false;
      this.rootStore.errorHandler.logUnexpectedError(e, 'Failed to load shelf data');
    }
  }
  ...
  async maybeLoadMore(categoryId: string) {
    ...
    const category = this.shelfCategories.find(c => c.parsedId === categoryId);
    ...
    if (category) {
      ...
      if (items) {
        runInAction(() => {
          items.map((item: ChildrenOfItemResponseItem | HomeChildItem) => getShelfItem(item, category.id));
          category.items = [...category.items, ...items.map(i => getShelfItem(i, category.parsedId))];
  ...
  getLimitOverridesGraph(): HomeGraphOverridesReq[] {
    return this.shelfCategories
      .filter(category => category.parsedId !== this.voiceCategoryId && category.items.length > FIRST_PAGE_SIZE)
      .map(category => ({
  ...
  private getVoiceCategory(parsedId: string): Category | undefined {
    const currentVoiceCategory = this.shelfCategories.find(
      (oldCategory: Category) => oldCategory.parsedId === this.voiceCategoryId && oldCategory.items.length > 0,
    );
  ...
  private getShelfDataFromCategoryItems(category: ShelfCategoryItem): Category {
    ...
    const children = category.children.map((item: HomeChildItem) => getShelfItem(item, id));
  ...
  private parseCategoriesFromDataStores(): Category[] {
    const categoriesFromDataStores = this.rootStore.homeItemsStore.homeItems.map(category =>
    ...
    categoriesFromDataStores
      .filter(category => category.parsedId !== this.voiceCategoryId)
      .forEach(category =>
        this.rootStore.childItemStore.prepareForPagination(category.id, category.items.length, category.total),
      );
   ...
   */
  private formatShelfData(category: HomeItem, childData: ShelfItem[], total: number) {
    const id = isHomeEndpointItemType(category) ? category.uri : category.id;
    const parsedId = parseCategoryId(id);
    const currentCategory = this.shelfCategories.find(c => c.id === id);

    const itemsVisible = this.getVisibleItems(parsedId, childData, currentCategory);
  ...
  private getVisibleItems(parsedId: string, childData: ShelfItem[], category?: Category) {
  ...
  populateVoice({ items, id }: VoiceResponse): void {
    if (items && Array.isArray(items)) {
      const voiceItems = items.map((item: VoiceItem) => getVoiceShelfItemType(item, id));

      const voiceCategory = this.shelfCategories.find(category => category.parsedId === id);

      if (voiceCategory && voiceItems.length) {
        voiceCategory.items = this.shouldShowVoiceDefault
          ? [getCategoryItems(voiceCategory, this.shouldShowVoiceDefault)[0], ...voiceItems]
          : voiceItems;
  ...
  get shelfItems(): ShelfItem[] {
    return this.shelfCategories.reduce((shelfItems: ShelfItem[], category: Category) => {
      const categoryItems = [...getCategoryItems(category, this.shouldShowVoiceDefault), ...getMoreItem(category)];
  ...
  getItemsByCategory(inputCategory: string) {
    const byCategory = this.shelfCategories.find(category => category.parsedId === inputCategory);
