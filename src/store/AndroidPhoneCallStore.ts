...
class AndroidPhoneCallStore implements PhoneCallStore {
  ...
  onMiddlewareEvent(msg: AndroidPhoneCallMessage) {
    ...
    switch (msg.type) {
      case 'com.spotify.superbird.phone.state':
        if (msg.payload) {
          this.updatePhoneCallStatus(msg.payload.state, msg.payload.phone_number, msg.payload.display_name);
        }
  ...
  async loadImage(phoneNumber: string) {
    try {
      ...
      this.rootStore.errorHandler.logUnexpectedError(e, 'Failed to load image data');
    }
  }
  ...
  updatePhoneCallStatus(status: PhoneCallState, phoneNumber: string, phoneDisplayName: string) {
