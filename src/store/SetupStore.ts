...
class SetupStore {
  ...
  constructor(rootStore: RootStore, socket: Socket) {
    ...
    this.rootStore.remoteControlStore.onInterappDisconnect(() => {
      this.handleSetupUiEvent(SetupUiEvent.CONNECTION_FAIL_DURING_CRITICAL_DOWNLOAD);
    });
  ...
  handleSetupUiEvent(event: SetupUiEvent) {
    ...
    switch (event) {
      ...
      case SetupUiEvent.SETUP_FINISHED_PHONE:
        ...
        } else if (otaStore.criticalUpdate && this.currentStep === SetupView.CONNECTED) {
          this.setupStep = SetupView.UPDATING;
        }
        break;
      case SetupUiEvent.SETUP_NAVIGATION_PHONE:
        if (this.currentStep === SetupView.START_SETUP) {
          this.setupStep = otaStore.criticalUpdate ? SetupView.UPDATING : SetupView.CONNECTED;
        }
        break;
      ...
      case SetupUiEvent.CRITICAL_OTA:
        if (this.currentStep === SetupView.CONNECTED && otaStore.autoUpdatable) {
          this.setupStep = SetupView.UPDATING;
        } else if (
      ...
      case SetupUiEvent.CONNECTION_FAIL_DURING_CRITICAL_DOWNLOAD:
        ...
        } else if (this.currentStep === SetupView.CONNECTED && otaStore.criticalUpdate) {
          this.setupStep = SetupView.WAITING;
        }
        break;
      default:
  ...
  get shouldShowSetup(): boolean {
    if ([SetupView.UPDATING, SetupView.WAITING, SetupView.FAILED].includes(this.currentStep)) {
      return true;
    }
