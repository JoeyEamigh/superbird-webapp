...
class DevOptionsStore {
  ...
  get alertDismissTime() {
    return this.rootStore.airVentInterferenceController.windAlertBannerUiState.dismissTime;
  }
  ...
  get alertDismissedAt(): string {
    const savedAt = this.rootStore.persistentStorage.getItem(WIND_NOISE_ALERT_DISMISSED_KEY);
  ...
  setImageStoreCacheSize(size: number) {
    this.rootStore.imageStore.setCacheSize(this.rootStore.imageStore.cacheSize + size);
  }
