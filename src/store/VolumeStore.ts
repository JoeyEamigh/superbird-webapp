...
type VolumeEvent = {
  type: 'com.spotify.volume.volume_state' | 'com.spotify.superbird.volume.volume_state';
  payload: VolumeState;
};
...
class VolumeStore {
  ...
  constructor(rootStore: RootStore, socket: Socket, interappActions: InterappActions) {
  ...
  onMiddlewareEvent(msg: VolumeEvent) {
    if (msg.type === 'com.spotify.volume.volume_state' || msg.type === 'com.spotify.superbird.volume.volume_state') {
  ...
  maybeChangeVolume(delta: number) {
    if (!this.rootStore.sessionStateStore.carMode) {
      ...
      if (delta > 0) {
        this.localVolume = Math.min(this.localVolume + 1 / this.volumeSteps, 1);
        this.interappActions.increaseVolume();
      } else if (delta < 0) {
        this.localVolume = Math.max(this.localVolume - 1 / this.volumeSteps, 0);
  ...
  private startDebounceTimer() {
    ...
    this.debounceTimeoutId = window.setTimeout(() => {
      ...
      if (this.receivedVolume !== undefined && this.localVolume !== this.receivedVolume) {
        runInAction(() => {
          this.localVolume = this.receivedVolume;
        });
      }
