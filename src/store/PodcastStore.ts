...
import { ChildItemResponse, ChildrenOfItemResponseItem } from 'store/ChildItemStore';
...
const isValid = (response: GetPodcastResponse): boolean => response?.items && Array.isArray(response.items);

export const parseConsumptionOrder = (consumptionOrder?: string): ConsumptionOrder | undefined => {
  if (!consumptionOrder) {
    return undefined;
  }
...
const mapTrailer = action((trailer: GetPodcastResponseItem) => Object.assign(trailer, { isTrailer: true }));
...
class PodcastStore {
  ...
  constructor(interappActions: InterappActions, remoteConfigStore: RemoteConfigStore, errorHandler: ErrorHandler) {
  ...
  podcasts: ObservableMap<string, GetPodcastResponse> = new ObservableMap<string, GetPodcastResponse>();

  loading: ObservableMap<string, boolean> = new ObservableMap<string, boolean>();
  ...
  async loadFromBeginning(uri: string, minimumEpisodesToLoad: number): Promise<void> {
    ...
    const result = await tryActionNTimes({
      asyncAction: () =>
        this.fetchPodcast(uri, Math.max(this.getNumberOfItemsExcludingTrailer(uri), minimumEpisodesToLoad)),
      n: 3,
    });
    runInAction(() => {
  ...
  private async fetchPodcast(parentId: string, limit: number, offset?: number): Promise<GetPodcastResponse> {
    ...
    try {
      ...
      const response = await this.interappActions.getPodcast(parentId, limit, offset);
  ...
  async loadMore(uri: string, pageSize: number, currentIndex: number, minDistanceFromEnd): Promise<void> {
    if (
      !shouldFetchMore(
        minDistanceFromEnd,
    ...
    const response = await this.fetchPodcast(
      uri,
      getNextLimit(pageSize, this.getNumberOfItemsExcludingTrailer(uri), this.getTotalNumberOfItems(uri)),
      this.getNumberOfItemsExcludingTrailer(uri),
    );
  ...
  isNotRecentConsumptionOrder(uri: string): boolean {
    return this.podcasts.get(uri)?.consumption_order !== ConsumptionOrder.RECENT;
  }
  ...
  shouldShowLatestPlayedEpisode(uri: string): boolean {
    return this.isNotRecentConsumptionOrder(uri) && this.hasLatestPlayedUri(uri);
  }
  ...
  getPodcastItemsIncludingTrailer(uri): GetPodcastResponseItem[] {
    ...
    return this.remoteConfigStore.podcastTrailerEnabled && trailer ? [mapTrailer(trailer), ...items] : items;
  }
