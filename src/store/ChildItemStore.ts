...
export const isChildrenOfItemType = (
  ...
  return !!(
    (itemToBeDetermined as ChildrenOfItemResponseItem).id && (itemToBeDetermined as ChildrenOfItemResponseItem).uri
  );
};

const isValid = (response: ChildItemResponse): boolean => response?.items && Array.isArray(response.items);

class ChildItemStore {
  rootStore: RootStore;
  interappActions: InterappActions;

  parents: ObservableMap<string, ChildItemResponse> = new ObservableMap<string, ChildItemResponse>();
  ...
  async loadChildData(parentId: string, minimumEpisodesToLoad: number): Promise<void> {
    ...
    const result = await tryActionNTimes({
      asyncAction: () =>
        this.fetchChildrenOfItem(parentId, Math.max(this.getNumberOfItems(parentId), minimumEpisodesToLoad), 0),
  ...
  async loadMore(moreParams: MoreParams): Promise<ChildrenOfItemResponseItem[] | undefined> {
    const { parentId, pageSize, currentIndex, numberOfItemsLoaded, totalNumberOfItemsAvailable, minDistanceFromEnd } =
      moreParams;
    if (
      !shouldFetchMore(minDistanceFromEnd, numberOfItemsLoaded, currentIndex, totalNumberOfItemsAvailable) ||
    ...
    try {
      const response = await this.fetchChildrenOfItem(
        parentId,
        getNextLimit(pageSize, numberOfItemsLoaded, totalNumberOfItemsAvailable),
        numberOfItemsLoaded,
      );
      ...
      this.rootStore.errorHandler.logUnexpectedError(e, `loadMore from getChildrenOfItem failed for ${parentId}`);
    }
    return toReturn;
  }
  ...
  private async fetchChildrenOfItem(parentId: string, limit: number, offset: number): Promise<ChildItemResponse> {
    let toReturn: ChildItemResponse = { items: [], total: 0, success: false };

    try {
      this.setLoading(parentId, true);

      const result = await this.interappActions.getChildrenOfItem(parentId, limit, offset);
      ...
      this.rootStore.errorHandler.logUnexpectedError(e, 'Failed to get children of item');
    } finally {
  ...
  async getChildItems(parentId: string, numberOfItemsToLoad): Promise<ChildItemResponse | undefined> {
