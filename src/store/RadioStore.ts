...
const uniqueByIdentifier = (items: QueueItem[]): QueueItem[] => {
  const seen = {};
  return items.filter(item => (seen[item.identifier] ? false : (seen[item.identifier] = true)));
};
...
class RadioStore {
  ...
  get currentRadioTracks(): TrackItem[] {
    ...
    return [...uniqueByIdentifier(queueStore.previous), queueStore.current, ...uniqueByIdentifier(queueStore.next)]
  ...
  playRadioItem(id: string): void {
    ...
    const queueItem = [...queueStore.previous, ...queueStore.next].find(item => item.identifier === id);
