...
import { FIRST_PAGE_SIZE, MoreParams, ShelfCategoryItem } from 'store/ShelfStore';
...
export const isHomeEndpointItemType = (
  ...
  return !!((itemToBeDetermined as HomeEndpointItem).uri && (itemToBeDetermined as HomeEndpointItem).children);
};

export const isGraphHomeItemType = (itemToBeDetermined: HomeItem): itemToBeDetermined is HomeGraphItem => {
...
class HomeItemsStore {
    ...
    const { result, success } = await tryActionNTimes({
      asyncAction: () => {
        return this.rootStore.remoteConfigStore.graphQLShelfEnabled
          ? this.queryHomeItems(this.rootStore.shelfStore.getLimitOverridesGraph())
  ...
  async fetchHomeItems(limitOverrides: HomeOverridesRequest): Promise<GetHomeResponse> {
    let toReturn: GetHomeResponse = { result: [], success: false };
    try {
      this.loading = true;
      const response = await this.interappActions.getHome(FIRST_PAGE_SIZE, limitOverrides);
      if (response.items) {
  ...
  async queryHomeItems(limitOverrides: HomeGraphOverridesReq[]): Promise<GetHomeResponse> {
    let toReturn: GetHomeResponse = { result: [], success: false };
    try {
      this.loading = true;
      const response = await this.interappActions.queryHome(FIRST_PAGE_SIZE, limitOverrides);
  ...
  async queryMore(moreParams: MoreParams): Promise<HomeChildItem[] | undefined> {
    const { parentId, pageSize, currentIndex, numberOfItemsLoaded, totalNumberOfItemsAvailable, minDistanceFromEnd } =
      moreParams;
    if (!shouldFetchMore(minDistanceFromEnd, numberOfItemsLoaded, currentIndex, totalNumberOfItemsAvailable)) {
      return undefined;
    }
    ...
    try {
      ...
      this.rootStore.errorHandler.logUnexpectedError(e, `query more home failed for ${parentId}`);
    }
    return toReturn;
  }
