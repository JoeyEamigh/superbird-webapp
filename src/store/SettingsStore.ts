...
import createSubmenuUiState, { SubmenuUiState } from 'component/Settings/Submenu/SubmenuUiState';
...
import createPhoneCallsUiState, { PhoneCallsUiState } from 'component/Settings/PhoneCalls/PhoneCallsUiState';
...
import createTipsOnDemandUiState, { TipsOnDemandUiState } from 'component/Settings/TipsOnDemand/TipsOnDemandUiState';
...
export type MenuItemId = MainMenuItemId | OptionsMenuItemId | AboutMenuItemId | RestartMenuItemId;
...
class SettingsStore {
  ...
  constructor(rootStore: RootStore, middlewareActions: MiddlewareActions, socket: Socket) {
    ...
    this.tipsOnDemandUiState = createTipsOnDemandUiState(this, rootStore.tipsStore, socket);

    this.unavailableSettingsBannerUiState = createUnavailableSettingsBannerUiState(
  ...
  get highlightedItem(): View | undefined {
    return this.currentView.rows ? this.currentView.rows[this.currentView.index] : undefined;
  }

  isMainMenuItemDisabled(disabledOffline: boolean | undefined): boolean {
    const { remoteControlStore, sessionStateStore, permissionsStore, viewStore } = this.rootStore;
    return (
      disabledOffline === true &&
    ...
    switch (this.currentView.id) {
      ...
      case MainMenuItemId.SETTINGS_ROOT:
        if (this.highlightedItem?.id === MainMenuItemId.MIC) {
          ...
          if (this.isMainMenuItemDisabled(this.highlightedItem.disabledOffline)) {
      ...
      case MainMenuItemId.PHONE_CONNECTION:
        if (phoneConnectionStore.phoneConnectionModal === PhoneConnectionModalView.FORGET_PHONE_CONFIRM) {
          if (phoneConnectionStore.phoneToConnectOrForget && phoneConnectionStore.forgetConfirmationIsActive) {
            settingsUbiLogger.logForgetConfirmDialPress(phoneConnectionStore.phoneToConnectOrForget.name);
            phoneConnectionStore.submitForget(phoneConnectionStore.phoneToConnectOrForget);
          } else {
            settingsUbiLogger.logForgetCancelDialPress();
            phoneConnectionStore.dismissModal();
          }
        } else if (phoneConnectionStore.phoneConnectionContextMenuUiState.phoneMenuShowing) {
  ...
  handleDialRight() {
    const { overlayController, phoneConnectionStore, bluetoothStore } = this.rootStore;
  ...
  handleDialLeft() {
    ...
    } else if (phoneConnectionStore.phoneConnectionModal === PhoneConnectionModalView.FORGET_PHONE_CONFIRM) {
  ...
  handleBack() {
    ...
    if (this.currentIsPhoneConnection && phoneConnectionStore.phoneConnectionModal !== undefined) {
      if (phoneConnectionStore.phoneConnectionModal === PhoneConnectionModalView.FORGET_PHONE_CONFIRM) {
        settingsUbiLogger.logConfirmForgetBackButton();
        phoneConnectionStore.phoneConnectionContextMenuUiState.setNewMenuIndex(0);
      } else {
