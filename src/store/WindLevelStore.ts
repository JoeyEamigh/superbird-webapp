...
class WindLevelStore {
  ...
  get alertDisabled(): boolean {
    return Boolean(JSON.parse(this.rootStore.persistentStorage.getItem(WIND_NOISE_ALERT_DISABLED_KEY) || 'false'));
  }
  ...
  isGoingOverThreshold(oldWindLevel: number, newWindLevel: number) {
    this.isOverThreshold = oldWindLevel < this.windAlertOnThreshold && newWindLevel >= this.windAlertOnThreshold;
  }

  toggleAlertDisabledByUser() {
    this.rootStore.persistentStorage.setItem(WIND_NOISE_ALERT_DISABLED_KEY, String(!this.alertDisabled));
  }
