...
class ImageStore {
  ...
  private getAnyImage(imageId: string): string | undefined {
    return this.imagesMap.get(ImageScale.BIG)?.get(imageId) || this.imagesMap.get(ImageScale.SMALL)?.get(imageId);
  }

  private shouldCalculateColor(imageId: string): boolean {
    return (this.getAnyImage(imageId) && !this.colorsMap.get(imageId) && !this.pendingColors.get(imageId)) || false;
  }
    ...
    if (this.shouldFetchImage(imageId, scale)) {
      ...
      try {
        ...
        runInAction(() => {
          ...
          if (this.requestedColors.get(imageId) && !get(this.colorsMap, imageId)) {
