...
export const setPresetsMutation = ({ serial, contextUri, slotIndex }: SetPresetArgs) =>
  compress(`
mutation {
  setPreset(input:{
    slotIndex: ${slotIndex},
...
export const getShelfQuery = (limit: number, overrides: HomeGraphOverridesReq[]) => {
  const overridesToString =
    overrides.length > 0
      ? `[${overrides.map(override => `{ id: "${override.id}", limit: ${override.limit} }`)}]`
      : '[]';
  return compress(`
query {
...
export const getShelfSectionQuery = (id: string, limit: number, offset: number) =>
  compress(`
query {
