...
import { getPresetsQuery, getShelfQuery, getShelfSectionQuery, getTipsOnDemandQuery } from 'middleware/Queries';
...
export const InterappMethods = {
  ...
  GetAvailablePodcastPlaybackSpeeds: 'com.spotify.get_available_podcast_playback_speeds',
  ...
  SummonDj: 'com.spotify.superbird.dj.summon',
...
class InterappActions {
  ...
  playUri(uri: string, featureIdentifier: string, interactionId?: string, skipToUri?: string, skipToUid?: string) {
  ...
  summonDj() {
    return this.doRequest(InterappMethods.SummonDj);
  }
  ...
  getPodcast(uri: string, limit?: number, offset?: number) {
    return this.doRequest(InterappMethods.GetPodcast, { limit, uri, offset }, false);
  }
  ...
  getHome(limit: number, overrides: HomeOverridesRequest): Promise<HomeResponsePayload> {
    return this.doRequest(InterappMethods.GetHome, { limit, limit_overrides: overrides }, false);
  }

  queryHome(limit: number, overrides: HomeGraphOverridesReq[]): Promise<GraphHomeResponsePayload> {
    return this.doGraphQLRequest(getShelfQuery(limit, overrides), false);
  }

  queryHomeMore(id: string, limit: number, offset: number): Promise<GraphHomeMoreResponsePayload> {
    return this.doGraphQLRequest(getShelfSectionQuery(id, limit, offset), false);
  }
   ...
   * @deprecated Use `playUri`
   */
  _playUriFromContext(contextUri: string, uri: string, featureIdentifier: string) {
