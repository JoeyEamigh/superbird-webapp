 ...
 * @private
 */
export function parseFromComponents(components: string[], format: URIFormat, query?: QueryParams): URI {
  let _current = 0;
  ...
  switch (part) {
    ...
    case URIParticle.APP: {
      ...
      for (let i = 0, len = args.length; i < len; ++i) args[i] = decodeComponent(args[i], format);
