...
export var semanticForegroundColors = _objectSpread(
  _objectSpread(_objectSpread({}, semanticTextColors), semanticEssentialColors),
  semanticDecorativeColors,
);
export var semanticColors = _objectSpread(
  _objectSpread(
    _objectSpread(_objectSpread({}, semanticBackgroundColors), semanticTextColors),
...
export var createBasicColorSet = function createBasicColorSet(backgroundValue, suggestedForegroundValue) {
  ...
  var foreground =
    suggestedForeground && calcAccessibleContrast(background, suggestedForeground) >= accessibleContrast
      ? suggestedForeground
      : getHighContrastColor(background);
  var foregroundIsLighter = foreground.luminosity() >= background.luminosity();
  var backgroundHighlight = foregroundIsLighter
    ? getColor(background.lightness(background.hsl().object().l - adjustment))
    : getColor(background.lightness(background.hsl().object().l + adjustment));
  var backgroundPress = foregroundIsLighter
    ? getColor(backgroundHighlight.lightness(backgroundHighlight.hsl().object().l - adjustment))
    : getColor(backgroundHighlight.lightness(backgroundHighlight.hsl().object().l + adjustment));
  return {
    background: {
      base: background.hex(),
