...
export var focusBorderBaseStyle = css(
  ['display:block;position:absolute;pointer-events:none;transition:border-color ', ' ', ';box-sizing:border-box;'],
  focusTransitionFade,
  focusTransitionType,
);
...
export var focusGapBorderStyle = function focusGapBorderStyle(borderRadius) {
  return css(
    ['', ' ', ' background:transparent;border-radius:', ';border:', 'px solid transparent;'],
    focusBorderBaseStyle,
...
export var focusBottomBorderStyle = function focusBottomBorderStyle(bottomBase) {
  return css(
    ['', ' bottom:', 'px;border-bottom:', 'px solid transparent;width:100%;'],
    focusBorderBaseStyle,
    bottomBase - focusBorderWidth * 2,
    focusBorderWidth,
  );
};
...
export var focusGapBorder = function focusGapBorder(borderRadius) {
  ...
  return css(
    ...
    isUsingKeyboard &&
      css(
        ['&::after{', " content:'';}&:focus::after{border-color:", ';}', ''],
        focusGapBorderStyle(borderRadius),
        focusBorderColor,
        showUsingProp && css(['&::after{border-color:', ';}'], focusBorderColor),
      ),
  );
};
...
export var focusBottomBorder = function focusBottomBorder() {
  ...
  return css(
    ...
    isUsingKeyboard &&
      css(
        ["&::after{content:'';", '}&:focus::after{border-color:inherit;}', ''],
        focusBottomBorderStyle(bottomBase),
        showUsingProp && css(['&::after{border-color:inherit;}']),
      ),
  );
};
