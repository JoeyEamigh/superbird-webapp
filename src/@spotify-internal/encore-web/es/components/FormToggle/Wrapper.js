...
export var Wrapper = styled.span.withConfig({
  ...
  var checkedStyles = css(
    ['background-color:', ';', "{right:2px;left:auto;[dir='rtl'] &{right:auto;left:2px;}}"],
    cssColorValue(props.semanticColor),
    Indicator,
  );

  // unchecked means toggle is in the "off" position
  var uncheckedStyles = css(
    ['background-color:', ';', "{left:2px;right:auto;[dir='rtl'] &{right:2px;left:auto;}}"],
  ...
  return css(
    [
      'border-radius:',
      'px;block-size:',
      ';position:relative;min-inline-size:',
      ';',
      ';input:focus ~ &&{',
      '}',
      ' input:active ~ &&{',
    ...
    spacer16,
    wrapperWidth,
    focusGapBorder(''.concat(borderRadius + focusBorderWidth, 'px'), props.isUsingKeyboard),
    focusStyles,
    props.isFocus && css(['input ~ &&{', '}'], focusStyles),
    activeStyles,
    props.isActive && css(['input ~ &&{', '}'], activeStyles),
    disabledStyles,
    uncheckedStyles,
    checkedStyles,
  );
