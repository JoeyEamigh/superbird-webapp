...
export var Button = styled(
  /*#__PURE__*/ React.forwardRef(function (_ref, ref) {
  ...
  var hoverStyles = css(
    ['', ' border-color:', ';', ''],
    !fullWidth && buttonStylesHover,
    borderHoverColor,
    semanticColor === 'textSubdued' && css(['color:', ';'], cssColorValue(semanticColors.textBase)),
  );
  var activeStyles = css(['', ' border-color:', ';'], buttonStylesActive, borderDefaultColor);
  var disabledStyles = css(
    ['', ' border-color:', ';&::after{border-color:transparent;}'],
    buttonStylesDisabled,
    borderDefaultColor,
  );
  ...
  return css(
    [
      ...
      'px solid ',
      ';color:',
      ';min-line-size:0;min-block-size:',
    ...
    buttonStylesBase(buttonSize, useBrowserDefaultFocusStyle, status),
    padding,
    !useBrowserDefaultFocusStyle && focusGapBorder(buttonBorderRadius, isUsingKeyboard, focus),
    borderWidth,
    borderDefaultColor,
    cssColorValue(semanticColor),
    controlMinBlockSize[buttonSize],
    overflowWrapFlexText(),
    fullWidth && css(['inline-size:100%;']),
    hoverStyles,
    hover && hoverStyles,
    activeStyles,
    active && activeStyles,
