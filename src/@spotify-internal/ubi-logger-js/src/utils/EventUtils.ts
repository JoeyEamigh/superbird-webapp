...
export function getParentPath(parentAbsoluteLocation: EventAbsoluteLocation | undefined): PathNode[] {
  return parentAbsoluteLocation
    ? (flatten(parentAbsoluteLocation.locations.map(location => location.pathNodes)) as PathNode[])
    : [];
}

export function getParentSpecVersions(parentAbsoluteLocation: EventAbsoluteLocation | undefined): string[] {
  return parentAbsoluteLocation
    ? (flatten(parentAbsoluteLocation.locations.map(location => location.specVersion)) as string[])
    : [];
}

export function getParentSpecModes(parentAbsoluteLocation: EventAbsoluteLocation | undefined): string[] {
  return parentAbsoluteLocation
    ? (flatten(parentAbsoluteLocation.locations.map(location => location.specMode)) as string[])
    : [];
}
