...
export const useSwiperDial = (args: Args) => {
  ...
  useEffect(() => {
    ...
    const disposer = autorun(() => {
      if (args.selectedItemIndex !== undefined && args.selectedItemIndex > -1 && !dragging) {
        if (swiperRef.current) {
          swiperRef.current.slideTo(args.selectedItemIndex, args.animateSliding ? transitionDurationMs : 0);
