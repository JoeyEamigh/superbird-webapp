...
class ModalUbiLogger {
  ...
  logStandbyImpression = () => {
    const event = createCarSettingsPowerModalCarthingosEventFactory().impression();
    this.ubiLogger.logImpression(event);
  };

  logStandbyConfirm = () => {
    const event = createCarSettingsPowerModalCarthingosEventFactory().powerOffButtonFactory().hitPowerOff();
    this.ubiLogger.logInteraction(event);
  };

  logStandbyCancel = () => {
    const event = createCarSettingsPowerModalCarthingosEventFactory().cancelButtonFactory().hitUiNavigateBack();
    this.ubiLogger.logInteraction(event);
  };

  logOtaNonCriticalImpression = () => {
    const event = createCarNonCriticalOtaModalCarthingosEventFactory().impression();
    this.ubiLogger.logImpression(event);
  };

  logSetupCriticalOtaImpression = () => {
    const event = createCarSetupCriticalOtaCarthingosEventFactory().impression();
    this.ubiLogger.logImpression(event);
  };
  ...
  logNoConnectionModalImpression = () => {
    const event = createCarNoBluetoothConnectionCarthingosEventFactory().impression();
    this.ubiLogger.logImpression(event);
  };
  ...
  logGoToPhoneListClick = () => {
    const event = createCarNoBluetoothConnectionCarthingosEventFactory().showPairedPhonesButtonFactory().hitUiNavigate({
      destination: MainMenuItemId.PHONE_CONNECTION,
    });
    this.ubiLogger.logInteraction(event);
  };
  ...
  logNeedPremiumModalShown = () => {
    const event = createCarNeedPremiumModalCarthingosEventFactory().impression();
    this.ubiLogger.logImpression(event);
  };
}
