...
class VoiceUbiLogger {
  ...
  constructor(ubiLogger: UbiLogger) {
    ...
    this.carVoiceCarthingosEventFactory = createCarVoiceViewCarthingosEventFactory();
  }
  ...
  logBackDismissVoiceOverlay = () => {
    const event = this.carVoiceCarthingosEventFactory.backButtonFactory().keyStrokeUiHide();
    this.ubiLogger.logInteraction(event);
  };

  logErrorImpression = (reason: string) => {
    const event = this.carVoiceCarthingosEventFactory.errorFactory({ reason }).impression();
    this.ubiLogger.logImpression(event);
  };

  logErrorTryAgain = (reason: string) => {
    const event = this.carVoiceCarthingosEventFactory.errorFactory({ reason }).hitRetry();
    this.ubiLogger.logInteraction(event);
  };
}
