...
class PhoneCallUbiLogger {
  ...
  logAnswerButtonClicked = () => {
    const event = this.phoneCallEventFactory.answerButtonFactory().hitAnswerPhoneCall();
    return this.ubiLogger.logInteraction(event);
  };

  logDeclineButtonClicked = () => {
    const event = this.phoneCallEventFactory.declineButtonFactory().hitDeclinePhoneCall();
    return this.ubiLogger.logInteraction(event);
  };

  logHangUpButtonClicked = () => {
    const event = this.phoneCallEventFactory.hangUpButtonFactory().hitHangUpPhoneCall();
    return this.ubiLogger.logInteraction(event);
  };
}
