...
class TrackListUbiLogger {
  ...
  logImpression = (contextUri: string) => {
    const event = TrackListUbiLogger.createEventFactory(contextUri).impression();
    this.ubiLogger.logImpression(event);
  };

  logTracklistItemImpression = (contextUri: string, trackUri: string, position: number) => {
  ...
  logContextRemoveLike = (contextUri: string) => {
    const event = TrackListUbiLogger.createEventFactory(contextUri).saveButtonFactory().hitRemoveLike({
      itemNoLongerLiked: contextUri,
    });
    this.ubiLogger.logInteraction(event);
  };

  logAddToQueueDialLongPress = (index: number, uri: string, contextUri: string) => {
    const event = TrackListUbiLogger.createEventFactory(contextUri)
  ...
  logTrackRowClicked = (index: number, uri: string, contextUri: string): string => {
    const event = TrackListUbiLogger.createEventFactory(contextUri)
      .trackRowFactory({ position: index, uri: uri })
  ...
  logDialPressTrackRow = (index: number, uri: string, contextUri: string): string => {
    const event = TrackListUbiLogger.createEventFactory(contextUri)
  ...
  logBackButtonPressed = (contextUri: string) => {
    const event = TrackListUbiLogger.createEventFactory(contextUri).backButtonFactory().keyStrokeUiNavigateBack();
    this.ubiLogger.logInteraction(event);
  };
