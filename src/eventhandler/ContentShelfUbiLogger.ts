...
class ContentShelfUbiLogger {
  ...
  constructor(ubiLogger: UbiLogger) {
    ...
    this.carContentShelfCarthingosEventFactory = createCarContentShelfCarthingosEventFactory();
  }
  ...
  logVoiceMutedBannerImpression = () => {
    const event = this.carContentShelfCarthingosEventFactory.voiceMutedBannerFactory().impression();
    this.ubiLogger.logImpression(event);
  };
  ...
  logDialButtonPressed = (category: string, index: number, uri?: string): string | undefined => {
    if (uri) {
      const event = this.carContentShelfCarthingosEventFactory
  ...
  logBackButtonPressed = () => {
    const event = this.carContentShelfCarthingosEventFactory.backButtonFactory().keyStrokeUiNavigateBack();
    this.ubiLogger.logInteraction(event);
  };

  logShelfArtworkImpression = (category: string, position: number, uri: string) => {
    const event = this.carContentShelfCarthingosEventFactory
      .categoryShelfRowFactory({ uri: category })
  ...
  logShelfItemClicked = (category: string, index: number, uri: string): string => {
    const event = this.carContentShelfCarthingosEventFactory
      .categoryShelfRowFactory({ uri: category })
