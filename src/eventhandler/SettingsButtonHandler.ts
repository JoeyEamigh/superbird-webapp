...
}) => ![AppView.LOGO, AppView.ONBOARDING].includes(appView) && !isPhoneCall && !isShowingPromo;

const reactToSettingsButton = (hardwareEvents: HardwareEvents, rootStore: RootStore) => {
  const {
    overlayController,
    settingsStore,
    viewStore,
