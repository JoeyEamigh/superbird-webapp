...
class ErrorHandler {
  ...
  logUnexpectedError = (error: Error, prefix?: string) => {
    ...
    if (error instanceof InterappError) {
      this.logInterappError(new InterappError(error.message, error.method, error.args));
    } else {
  ...
  onUnhandledRejection = (e: PromiseRejectionEvent) => {
    ...
    if (e.reason.message && e.reason.method && e.reason.args) {
      this.logInterappError(new InterappError(e.reason.message, e.reason.method, e.reason.args));
    } else {
      this.logError(new Error(`Promise rejected: ${e.reason.toString()}`), ErrorCategory.UNHANDLED_REJECTION);
    }
  };
}
