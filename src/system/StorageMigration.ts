...
import { OLD_TIPS_DISABLED_KEY, TIPS_ACTIVE_KEY } from '../component/Npv/Tips/TipsUiState';
...
export const BROWSER_LOCAL_STORAGE_MIGRATED_TO_MW_KEY = 'browser_storage_migrated_to_mw';
...
export const migrateFromLocalStorage = (
  ...
  when(
    ...
    () => {
      if (
        newStorage.getItem(BROWSER_LOCAL_STORAGE_MIGRATED_TO_MW_KEY) !== 'true' &&
        newStorage.getItem(newKey) === null
      ) {
        const old = globalThis.localStorage.getItem(oldKey);
        if (old !== null) {
...
export const migrateAllFromLocalStorage = (newStorage: SeedableStorageInterface) => {
