...
class MockHardwareEvents {
  ...
  triggerDialRight = (times = 1): void => {
    this.doTimes(() => this.hardwareEvents.notifyListeners(HardwareEvent.DIAL_RIGHT), times);
  };

  triggerDialLeft(times = 1): void {
    this.doTimes(() => this.hardwareEvents.notifyListeners(HardwareEvent.DIAL_LEFT), times);
  }

  triggerDialPress(times = 1): void {
    this.doTimes(() => this.hardwareEvents.notifyListeners(HardwareEvent.DIAL_PRESS), times);
  }
  ...
  triggerBack(times = 1): void {
    this.doTimes(() => this.hardwareEvents.notifyListeners(HardwareEvent.BACK), times);
  }
