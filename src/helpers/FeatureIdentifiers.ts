...
export const FEATURE_IDENTIFIER_ALTERNATIVE_SEARCH = 'car-thing-alternative-search';
...
const isFirstVoiceItem = (item: ShelfItem, shelfStore: ShelfStore) => {
  return shelfStore.shelfController.getIndexOfItemInCategory(item.uri, item.category) === 0;
};
...
export const setTrackViewFeatureIdentifier = (newFeatureIdentifier: string | null) => {
...
export const getTrackViewFeatureIdentifier = (contextUri: string) => {
  ...
  return trackViewFeatureIdentifierMap[uri.type] || defaultTrackViewFeatureIdentifier;
};
...
export const setTrackViewVoiceFeatureIdentifier = (
  ...
  if (isFirstVoiceItem(selectedItem, shelfStore) && isPlayRequest(voiceStore)) {
    ...
    setTrackViewFeatureIdentifier(
      isShowIntent(voiceStore.intent) ? FEATURE_IDENTIFIER_SHOW_ME : FEATURE_IDENTIFIER_ALTERNATIVE_SEARCH,
    );
  }
};
...
export const getShelfFeatureIdentifierForTrackOrEpisode = (
  ...
  if (selectedItem.category === VOICE_IDENTIFIER) {
    ...
    // Depending on the user's intent we use different feature identifiers.
    return isShowIntent(voiceStore.intent) ? FEATURE_IDENTIFIER_SHOW_ME : FEATURE_IDENTIFIER_ALTERNATIVE_SEARCH;
  }
