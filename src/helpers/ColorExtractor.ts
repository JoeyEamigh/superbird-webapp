...
export const extractColor = (base64ImageContent: string) => {
  return fetchImage(`data:image/jpeg;base64,${base64ImageContent}`).then(domImage => {
...
const ensureContrastRatio = (hex: string) => {
  ...
  while (contrast(`#${convert.hsl.hex(hsl)}`, '#000000') < minBackgroundContrast && hsl[2] < 100) {
    hsl[2] += 1;
  }

  // Subtract lightness (L in HSL) until accessible against white.
  while (contrast(`#${convert.hsl.hex(hsl)}`, '#FFFFFF') < minTextContrast && hsl[2] > 0) {
    hsl[2] -= 1;
  }
