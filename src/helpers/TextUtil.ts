export const firstLetterUpperCase = function firstLetterUpperCase(text: string): string {
  if (!text) return text;
  return text.substr(0, 1).toUpperCase() + text.substr(1);
};

export const parsePinCodeSpacing = function parsePinCodeSpacing(text: string | undefined) {
  if (!text) return text;
