...
export const isYourEpisodesUri = (uri: string) => isCollectionUri(uri) && uri.includes(':your-episodes');

export const isNewEpisodesUri = (uri: string) => isCollectionUri(uri) && uri.includes(':podcasts:episodes');
...
export const isDjUri = (uri: string) => {
  return uri === DJ_URI;
};
...
export const maskUsernames = (payload: Object) => {
  ...
  return Object.entries(payload ?? {}).reduce((acc, [key, value]) => ({ ...acc, [key]: mask(value) }), {});
};
