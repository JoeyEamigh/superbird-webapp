...
export class Socket {
  ...
  post(body: Record<string, any>): void {
    const isInterappMessage = body.method && INTERAPP_METHODS.includes(body.method);
  ...
  handleCallResult(msg: any) {
    const payloadReponseSize = msg.payload ? JSON.stringify(msg.payload).length : 0;
    if (msg.type === 'call_result') {
      if (this.shouldLogRequests) {
        this.requestLogger.requestFinished(msg.msgId, payloadReponseSize, Date.now(), true);
      }
    } else if (msg.type === 'call_error') {
      if (this.shouldLogRequests) {
        this.requestLogger.requestFinished(msg.msgId, payloadReponseSize, Date.now(), false);
      }
    }
  }
