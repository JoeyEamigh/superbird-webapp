...
const MainMenu = () => {
  ...
  useEffect(() => settingsUbiLogger.logMainMenuImpression(), [settingsUbiLogger]);

  useEffect(() => {
    const effect = () => {
      if (swiper && settingsStore.currentView.id === MainMenuItemId.SETTINGS_ROOT) {
      ...
      >
        {settingsStore.rows?.map((item, index) => (
          <SwiperSlide key={item.id}>
            <MainMenuItem item={item} active={settingsStore.currentView.index === index} />
          </SwiperSlide>
        ))}
      </SwiperComponent>
    </div>
