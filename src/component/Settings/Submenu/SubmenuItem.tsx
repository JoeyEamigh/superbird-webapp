...
const OnOrOff = ({ id, isToggleOn, active }: { id: MenuItemId; isToggleOn: boolean; active: boolean }) => (
  <div
    className={classNames(styles.onOffToggle, isToggleOn ? styles.green : styles.white70, {
...
const SubmenuItem = ({ item, active }: Props) => {
  ...
  return (
    <div
      ...
      onClick={() => (disabled ? uiState.showUnavailableBanner() : uiState.handleSubmenuItemClicked(item))}
    >
      <Type name="canonBold">{label}</Type>
      {(() => {
        switch (type) {
          case 'toggle':
            return <OnOrOff active={active} id={id} isToggleOn={uiState.isToggleOn(item)} />;
          case 'key-value':
            return (
              <Type
