...
const TipsOnDemandError = ({ uiState }: { uiState: TipsOnDemandUiState }) => {
  ...
  return (
    <div className={styles.error}>
      ...
      <Type name="celloBook">
        You might have lost the connection to your phone or network. Try again in a little while.
      </Type>
      <Button onClick={uiState.handleConfirmError} type={ButtonType.BUTTON_PRIMARY}>
        OK
      </Button>
    </div>
  );
...
const TipsOnDemand = () => {
  ...
  useEffect(() => {
    ...
    uiState.handleMount();
    return () => uiState.handleUnmount(getSecondsSince(currentTipShown), getSecondsSince(viewOpened));
  }, [uiState]);
