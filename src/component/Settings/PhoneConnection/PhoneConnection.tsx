...
const PhoneConnection = () => {
  const { settingsStore, bluetoothStore, remoteControlStore, hardwareStore, phoneConnectionStore, ubiLogger } =
    useStore();
  ...
  useEffect(() => {
    ...
    runInAction(effect);
  }, [settingsStore.currentIsPhoneConnection, settingsStore.currentView.index, swiper]);
  ...
  const addPhoneItemIsActive = settingsStore.currentView.index === bluetoothStore.bluetoothDeviceList.length;

  return (
    <div className={classNames(styles.phoneConnection)}>
      <SubmenuHeader name={settingsStore.phoneConnectionView.label} icon={icon} />
      <SwiperComponent
        setWrapperSize
        direction="vertical"
        ...
        onActiveIndexChange={swiperInstance => settingsStore.handleSettingSetNewIndex(swiperInstance.activeIndex)}
        data-testid={`${MainMenuItemId.PHONE_CONNECTION}-submenu`}
      >
        {bluetoothStore.bluetoothDeviceList?.map((phone, index) => {
          const isCurrentDevice = phone.address === bluetoothStore.currentDevice?.address;
          const isConnected = remoteControlStore.isConnectedPhone(phone.address);
        ...
        <SwiperSlide
          className={classNames(styles.phoneConnectionItem, styles.addMorePhone, {
            [styles.active]: addPhoneItemIsActive,
            [styles.pressed]: pressedAddMoreItem || (addPhoneItemIsActive && hardwareStore.dialPressed),
          })}
      ...
      <Overlay show={phoneConnectionStore.phoneConnectionContextMenuUiState.phoneMenuShowing} appear={FROM.BOTTOM}>
