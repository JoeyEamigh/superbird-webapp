...
import PhoneConnectionStore, { Device, PhoneConnectionModalView } from 'store/PhoneConnectionStore';
...
class PhoneConnectionContextMenuUiState {
  ...
  get displayConnectionStatus(): string {
    return this.phoneConnectionStore.getPhoneConnectionDisplayStatus(this.phoneConnectionStore.phoneToConnectOrForget);
  }

  get isConnected(): boolean {
    return this.remoteControlStore.isConnectedPhone(this.phoneConnectionStore.phoneToConnectOrForget?.address);
  }
    ...
    switch (item) {
      case 'Connect':
        if (this.phoneConnectionStore.phoneToConnectOrForget) {
          this.phoneConnectionStore.handleSelectPhoneClick(this.phoneConnectionStore.phoneToConnectOrForget);
        }
        break;
      case 'Forget':
        this.phoneConnectionStore.setPhoneConnectionModal(PhoneConnectionModalView.FORGET_PHONE_CONFIRM);
        break;
      default:
        break;
    }
