...
const Licenses = () => {
  ...
  useEffect(() => settingsUbiLogger.logThirdPartyLicenseImpression(), [settingsUbiLogger]);
  ...
  useEffect(() => {
    ...
    async function fetchLicenses() {
      ...
      const [uiLicense, mwLicense, osLicense] = await Promise.all(filePromises.map(filePromise => filePromise.text()));
  ...
  function getItemHeight(itemIndex: number): number {
    ...
    return Math.ceil(allLicenseLines[itemIndex].length / charsPerLine) * LINE_HEIGHT;
  }
    ...
    <div className={styles.licenses}>
      <Type name="altoBold" className={styles.header}>
        ...
        <Type name="altoBook" className={styles.pretext}>
          For third-party software that you receive from Spotify which is licensed under a license that gives you the
          right to receive the source code, <br /> you can obtain a copy at this website: https://github.com/spotify
        </Type>
      </Type>
      {allLicenseLines.length ? (
        ...
        >
          {({ index, style, data }) => <div style={{ ...style, wordBreak: 'break-all' }}>{data[index]}</div>}
        </VariableSizeList>
      ) : (
