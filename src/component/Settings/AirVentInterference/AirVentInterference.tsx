...
const AirVentInterference = () => {
  ...
  const uiState = useStore().airVentInterferenceController.airVentInterferenceUiState;

  useEffect(() => {
    const onTouchEnd = () => {
      const scrollElement = aviContainerRef.current;
      if (scrollElement) {
        uiState.setAirVentInterferenceScrollStep((scrollElement.scrollTop / CONTENT_HEIGHT) * NUMBER_OF_SCROLL_STEPS);
      }
    };
      ...
      <div ref={aviContainerRef} className={styles.aviContainer}>
        ...
        <div className={styles.texts}>
          <p className={styles.intro}>
            Too much air flowing into your microphones will <br /> likely interfere with voice requests. When we detect
            an issue, <IconWind32 /> will appear at the top right corner of the screen. If this happens, here are some
            things to try:
          </p>
          <ul>
