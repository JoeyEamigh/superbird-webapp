...
const VoiceConfirmation = ({ intent, action }: Props): JSX.Element => {
  switch (intent) {
    ...
    case ADD_TO_COLLECTION_INTENT:
      if (action === SAVE_TO_COLLECTION_PODCAST_ACTION || action === SAVE_TO_COLLECTION_EPISODE) {
        return <IconCheckAlt className={styles.confirmationIcon} iconSize={iconSize} />;
      }
      return <IconHeartAltActive className={styles.confirmationIcon} iconSize={iconSize} />;
    case SHUFFLE_ON_INTENT:
      return (
        <div data-testid="shuffle-confirmation">
          <IconShuffleActive className={styles.confirmationIcon} iconSize={iconSize} />
        </div>
      );
    case SHUFFLE_OFF_INTENT:
      return (
        <div data-testid="shuffle-confirmation">
          <IconShuffle className={styles.confirmationIcon} iconSize={iconSize} />
        </div>
      );
    case REPEAT_ON_INTENT:
      return <IconRepeat className={styles.confirmationIcon} iconSize={iconSize} />;
    case REPEAT_OFF_INTENT:
      return <IconRepeat className={styles.confirmationIcon} iconSize={iconSize} />;
    case REPEAT_ONE_INTENT:
      return <IconRepeatOne className={styles.confirmationIcon} iconSize={iconSize} />;
    case SET_PLAYBACK_SPEED_1X_INTENT:
      return <IconPlaybackSpeed1x className={styles.confirmationIcon} iconSize={iconSize} />;
    case SET_PLAYBACK_SPEED_1POINT2X_INTENT:
      return <IconPlaybackSpeed1point2x className={styles.confirmationIcon} iconSize={iconSize} />;
    case SET_PLAYBACK_SPEED_1POINT5X_INTENT:
      return <IconPlaybackSpeed1point5x className={styles.confirmationIcon} iconSize={iconSize} />;
    case MUTE_INTENT:
    ...
    default:
      ...
      return <IconCheck data-testid="default-confirmation" className={styles.confirmationIcon} iconSize={iconSize} />;
  }
};
