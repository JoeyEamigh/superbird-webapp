...
export function Listening({
  ...
  if (errorUiTitle) {
    ...
    content = (
      <Type name="brioBold" className={styles.fadeIn} dataTestId="voice-error-text" style={{ whiteSpace: 'pre-wrap' }}>
        {friendlyError || (
    ...
    content = (
      <Type name="brioBold" className={classnames(styles.textConfirmation, styles.fadeIn)}>
    ...
    content = (
      <div className={classnames(styles.voiceConfirmation, styles.centered)}>
        ...
        <Type name="altoBold" textColor="white" className={styles.voiceConfirmationText}>
          {(action && ACTION_TO_CONFIRMATION_TEXT[action]) || INTENT_TO_CONFIRMATION_TEXT[intent] || ''}
        </Type>
      </div>
    );
...
const ListeningContainer = () => {
  ...
  useEffect(() => {
    if (voiceStore.isError) {
      ubiLogger.voiceUbiLogger.logErrorImpression(voiceStore.error || voiceStore.friendlyError);
    }
  }, [voiceStore.isError, ubiLogger.voiceUbiLogger, voiceStore.error, voiceStore.friendlyError]);
  ...
  const { showingVoiceConfirmation, intent, errorUiTitle, errorUiSubtitle, error, friendlyError, isError, listening } =
    voiceStore;
