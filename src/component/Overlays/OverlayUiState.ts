...
class OverlayUiState {
  ...
  get shouldShowNoConnection(): boolean {
    const { remoteControlStore, bluetoothStore, settingsStore } = this.rootStore;

    return remoteControlStore.noConnectionAtAll && !bluetoothStore.pairing && !settingsStore.currentIsPhoneConnection;
  }
  ...
  get shouldShowNoContext(): boolean {
    const { playerStore, permissionsStore, sessionStateStore, viewStore } = this.rootStore;
    return Boolean(
  ...
  maybeShowNoConnection(): boolean {
    ...
    if (this.shouldShowNoConnection) {
      ...
      this.noConnectionModalDelayTimeoutId = window.setTimeout(this.showNoConnection, MODAL_DELAY);
      return true;
    } else if (!remoteControlStore.noConnectionAtAll && this.isShowing('no_connection')) {
      this.resetAndMaybeShowAModal();
    } else if (remoteControlStore.noConnectionAtAll && bluetoothStore.pairing && this.isShowing('no_connection')) {
      this.resetAndMaybeShowAModal();
    }
    return false;
  }
  ...
  maybeShowNoNetwork(): boolean {
    ...
    if (
      sessionStateStore.isOffline &&
      [View.CONTENT_SHELF, View.TRACKLIST, View.QUEUE].includes(viewStore.currentView) &&
  ...
  maybeShowPremiumRequired(): boolean {
    ...
    const shouldShowPremiumRequired = permissionsStore.canUseCarThing === false && !this.isShowing('settings');
    if (shouldShowPremiumRequired) {
      this.showOverlay('premium_required');
      return true;
    } else if (permissionsStore.canUseCarThing && this.isShowing('premium_required')) {
      this.resetAndMaybeShowAModal();
    }
    return false;
  }
  ...
  maybeShowLoginRequired(): boolean {
    ...
    const shouldShowNotLoggedIn = !sessionStateStore.isLoggedIn && !this.isShowing('settings');
    if (shouldShowNotLoggedIn) {
      this.showOverlay('login_required');
      return true;
    } else if (sessionStateStore.isLoggedIn && this.isShowing('login_required')) {
      this.resetAndMaybeShowAModal();
    }
    return false;
  }
  ...
  maybeShowLetsDrive(): boolean {
    if (this.shouldShowNoContext) {
      this.noContextModalDelayTimeoutId = window.setTimeout(this.showNoContext, MODAL_DELAY);
      return true;
  ...
  maybeShowBluetoothPairing(): boolean {
    const { bluetoothStore, settingsStore, remoteControlStore } = this.rootStore;
    if (bluetoothStore.pairing && !settingsStore.currentIsPhoneConnection) {
      this.showOverlay('bluetooth_pairing');
      return true;
    } else if (!bluetoothStore.pairing && remoteControlStore.interappConnected && this.isShowing('bluetooth_pairing')) {
      this.resetAndMaybeShowAModal();
    }
    return false;
  }
