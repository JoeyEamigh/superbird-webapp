...
import OverlayUiState, { DEFAULT_PRESET_TIMEOUT, Overlay } from 'component/Overlays/OverlayUiState';
...
export const createOverlayController = (rootStore: RootStore, ubiLogger: UbiLogger) => {
  return {
    ...
    handleDialPress(): void {
      switch (this.overlayUiState.currentOverlay) {
        case 'voice':
          if (rootStore.voiceStore.error || rootStore.voiceStore.friendlyError) {
            rootStore.voiceStore.retry();
          }
          break;
        case 'presets':
          rootStore.presetsController.presetsUiState.handleDialPress();
          window.setTimeout(() => this.overlayUiState.resetAndMaybeShowAModal(), PRESET_SELECT_TO_PLAY_TIMEOUT);
          break;
        case 'settings':
          rootStore.settingsStore.handleDialPress();
          break;
        case 'podcast_speed':
          if (rootStore.npvStore.podcastSpeedUiState.selectedItem && rootStore.playerStore.currentTrackUri) {
      ...
      switch (this.overlayUiState.currentOverlay) {
        ...
        case 'podcast_speed':
          if (rootStore.npvStore.podcastSpeedUiState.leftItem) {
            rootStore.npvStore.podcastSpeedUiState.updateSelectedItem(rootStore.npvStore.podcastSpeedUiState.leftItem);
          }
          break;
        case 'presets':
          this.showPresets();
      ...
      switch (this.overlayUiState.currentOverlay) {
        ...
        case 'podcast_speed':
          if (rootStore.npvStore.podcastSpeedUiState.rightItem) {
            rootStore.npvStore.podcastSpeedUiState.updateSelectedItem(rootStore.npvStore.podcastSpeedUiState.rightItem);
          }
          break;
        case 'presets':
          this.showPresets();
