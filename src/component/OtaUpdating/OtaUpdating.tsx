      ...
      {otaStore.error ? (
        <>
          ...
          <div className={styles.subtitle}>
            To restart the device and try again, please unplug the power cable, then plug it back in.
          </div>
        </>
      ) : (
        <>
          ...
          <div className={styles.subtitle}>Please keep Car Thing plugged in and powered on.</div>
          {otaStore.transferring && <div className={styles.progress}>{`${otaStore.transferPercent}%`}</div>}
        </>
      )}
    </div>
  );
