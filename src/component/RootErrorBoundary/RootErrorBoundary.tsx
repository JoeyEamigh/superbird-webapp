...
class RootErrorBoundary extends Component<PropsWithChildren<RootStoreProps>, State> {
  ...
  componentDidCatch(error: Error, { componentStack }: React.ErrorInfo) {
    ...
    errorHandler.logUnexpectedError(error, 'Fatal UI error causing device restart: ');
    ...
    return this.state.hasError ? (
      <div className={styles.dialog}>
        ...
        <div className={styles.description}>
          <p>Restarting in {this.state.counter > 0 ? this.state.counter : 0} seconds</p>
        </div>
      </div>
    ) : (
      this.props.children
