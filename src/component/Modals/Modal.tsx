...
const Modal = () => {
  ...
  return (
    <>
      ...
      <Overlay appear={FROM.FADE_IN} show={overlayController.isShowing('premium_required')}>
        <PremiumAccountRequired />
      </Overlay>
      <Overlay appear={FROM.FADE_IN} show={overlayController.isShowing('no_connection')}>
        <NoConnection />
      </Overlay>
      <Overlay appear={FROM.FADE_IN} show={overlayController.isShowing('login_required')}>
        <LoginRequired />
      </Overlay>
      <Overlay appear={FROM.FADE_IN} show={overlayController.isShowing('bluetooth_pairing')}>
        <BluetoothPairing />
      </Overlay>
      <Overlay appear={FROM.FADE_IN} show={overlayController.isShowing('non_supported_type')}>
        <NonSupportedType />
      </Overlay>
      <Overlay appear={FROM.FADE_IN} show={overlayController.isShowing('standby')}>
        <Standby />
      </Overlay>
      <Overlay show={overlayController.isShowing('save_preset_error')} appear={FROM.FADE_IN}>
        <SavingPresetFailed />
      </Overlay>
      <Overlay appear={FROM.FADE_IN} show={overlayController.isShowing('no_network')}>
        <NoNetwork />
      </Overlay>
    </>
  );
