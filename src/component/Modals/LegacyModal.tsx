...
const LegacyModal = ({ children }: PropsWithChildren<{}>) => {
  ...
  const handleModalOnClick = (e: React.MouseEvent<HTMLDivElement, MouseEvent>): void => {
    e.preventDefault();
    e.stopPropagation();
  };

  return (
    <div className={styles.legacyModal} data-testid="modal" onClick={uiState.handleBackdropOnClick}>
      <div className={styles.content} data-testid="modal-content" onClick={handleModalOnClick}>
        {uiState.isDismissible && (
          <div data-testid="modal-icon-dismissible" onClick={() => uiState.maybeShowAModal()}>
