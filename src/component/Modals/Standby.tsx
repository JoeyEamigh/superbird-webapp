...
const Standby = () => {
  ...
  useEffect(() => ubiLogger.modalUbiLogger.logStandbyImpression(), [ubiLogger.modalUbiLogger]);

  useEffect(() => overlayUiState.setPowerOffConfirmationIsActive(true), [overlayUiState]);
  ...
  const renderButtons = () => {
    return (
      <ButtonGroup style={{ width: '360px' }}>
        <Button
          type={overlayUiState.powerOffConfirmationIsActive ? ButtonType.BUTTON_PRIMARY : ButtonType.BUTTON_SECONDARY}
          onClick={handlePowerClick}
        >
          Power off
        </Button>
        <Button
          type={overlayUiState.powerOffConfirmationIsActive ? ButtonType.BUTTON_SECONDARY : ButtonType.BUTTON_PRIMARY}
