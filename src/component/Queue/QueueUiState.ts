...
class QueueUiState {
  ...
  get selectedItemIndex(): number {
    return this.queue.findIndex(item => this.selectedItem?.queue_index === item.queue_index);
  }
  ...
  get rightItem(): QueueItem | undefined {
    if (this.selectedItemIndex === this.queue.length - 1 || this.selectedItemIndex < 0) {
      return this.selectedItem;
    }
    return this.queue[this.selectedItemIndex + 1];
  }
  ...
  get headerBackground(): string {
    const colorChannels = get(this.colors, this.currentPlayingImageId) || [0, 0, 0];
  ...
  setSelectedItemOnQueueChange(): void {
    const previousInNewQueue = this.queue.find(item => item.identifier === this.selectedItem?.identifier);
    if (this.selectedItemIndex === 0 || this.selectedItem === undefined) {
      this.updateSelectedItem(this.queueStore.next[0]);
    } else if (this.selectedItemIndex > 0 && this.queueStore.isNewCurrent(this.selectedItem)) {
      this.updateSelectedItem(this.queueStore.next[0]);
  ...
  handleDraggedToIndex(index: number): void {
    const userDraggedToItem = this.queue.find((_, itemIndex) => index === itemIndex);
  ...
  handleDialPress(): void {
    if (this.selectedItem) {
      this.queueUbiLogger.logDialPressTrackRow(this.selectedItemIndex, this.selectedItem.uri);
