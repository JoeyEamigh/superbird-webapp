    ...
    >
      <div className={styles.imageContainer}>
        <div className={styles.image} data-testid={`queue-image-${item.uri}`}>
          <LazyImage uri={item.uri} size={96} scale={1.3} imageId={item.image_uri} isActive={isActive} />
        </div>
      </div>

      <div className={styles.trackInfo} data-testid={`queue-info-${item.uri}`}>
        <Type className={styles.title} name="canonBold" dataTestId={`${isActive ? 'queue-item-title' : ''}`}>
          {item.name}
        </Type>
        <Type name="balladBook" className={styles.subtitle} dataTestId={`${isActive ? 'queue-item-subtitle' : ''}`}>
          {item.artist_name}
        </Type>
      </div>
    </div>
