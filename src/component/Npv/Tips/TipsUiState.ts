...
class TipsUiState {
  ...
  constructor(
    ...
    this.playerStore.onTrackChange(() => {
      ...
      newTrackTimeoutId = window.setTimeout(() => this.setNewTrack(false), TIPS_TRACK_CHANGE_DELAY);
    });
  }

  get tipToShow(): Tip | undefined {
    if (this.tipsStore.tip && this.isNpvIdle && !this.isNewTrack && this.isTipsEnabled) {
      showTipTimeoutId = window.setTimeout(() => this.dismissVisibleTip(), TIPS_SHOW_TIME);
  ...
  private triggerNpvIdleTimer() {
    npvIdleTimeoutId = window.setTimeout(() => this.setNpvIdle(true), TIPS_INTERACTION_DELAY);
  }
  ...
  get tipsSettingOn(): boolean {
    return Boolean(JSON.parse(this.persistentStorage.getItem(TIPS_ACTIVE_KEY) || 'true'));
  }

  get isTipsEnabled(): boolean {
    return this.tipsSettingOn && !this.onboardingStore.isOnboardingOngoing;
  }
  ...
  toggleTipsEnabled() {
    this.persistentStorage.setItem(TIPS_ACTIVE_KEY, String(!this.tipsSettingOn));
