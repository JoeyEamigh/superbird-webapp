...
class OtherMediaUbiLogger {
  ...
  constructor(ubiLogger: UbiLogger) {
    ...
    this.otherMediaEventFactory = createCarNowPlayingViewOtherMediaCarthingosEventFactory();
  }
  ...
  logPlaySpotifyClicked() {
    const event = this.otherMediaEventFactory.playSpotifyButtonFactory().hitUiNavigate({ destination: 'Spotify' });
  ...
  logPlayButtonClicked(currentTrackUri: string) {
    const event = this.otherMediaEventFactory.controlButtonsFactory().playPauseButtonFactory().hitResume({
      itemToBeResumed: currentTrackUri,
    });
    return this.ubiLogger.logInteraction(event);
  }

  logSkipPrevClicked(currentTrackUri: string, currentPositionMs: number, currentTrackDuration: number) {
    const event = this.otherMediaEventFactory.controlButtonsFactory().skipPreviousButtonFactory().hitSkipToPrevious({
  ...
  logSkipNextClicked(currentTrackUri: string, currentPositionMs: number, currentTrackDuration: number) {
    const event = this.otherMediaEventFactory.controlButtonsFactory().skipNextButtonFactory().hitSkipToNext({
      itemToBeSkipped: currentTrackUri,
      positionMs: currentPositionMs,
      totalContentMs: currentTrackDuration,
    });
  ...
  logDecreaseVolumeClicked() {
    const event = this.otherMediaEventFactory.dialButtonFactory().rotateDecreaseVolume();
    this.ubiLogger.logInteraction(event);
  }

  logIncreaseVolumeClicked() {
    const event = this.otherMediaEventFactory.dialButtonFactory().rotateIncreaseVolume();
    this.ubiLogger.logInteraction(event);
  }

  logDialPlayClicked(currentTrackUri: string) {
    const event = this.otherMediaEventFactory.dialButtonFactory().keyStrokeResume({
      itemToBeResumed: currentTrackUri,
    });
    return this.ubiLogger.logInteraction(event);
  }
  ...
  logDialPauseClicked(currentTrackUri: string) {
    const event = this.otherMediaEventFactory.dialButtonFactory().keyStrokePause({
      itemToBePaused: currentTrackUri,
    });
    return this.ubiLogger.logInteraction(event);
  }
  ...
  logBackClicked(view: string) {
    const event = this.otherMediaEventFactory.backButtonFactory().keyStrokeUiNavigate({ destination: view });
    this.ubiLogger.logInteraction(event);
  }

  logWindNoiseImpression() {
    const event = this.otherMediaEventFactory.windNoiseBannerFactory().impression();
    this.ubiLogger.logImpression(event);
  }

  logWindNoiseDismissal() {
    const event = this.otherMediaEventFactory.windNoiseBannerFactory().dismissButtonFactory().hitUiHide();
    this.ubiLogger.logInteraction(event);
  }

  logWindNoiseLearnMore() {
    const event = this.otherMediaEventFactory.windNoiseBannerFactory().learnMoreButtonFactory().hitUiNavigate({
