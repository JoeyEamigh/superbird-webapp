...
export default class WindAlertBannerUiState {
  ...
  get shouldShowIcon(): boolean {
    return (
      this.windLevelStore.currentWindLevel >= this.windLevelStore.windAlertOnThreshold && !this.voiceStore.isMicMuted
    );
  }
  ...
  get shouldShowAlert(): boolean {
    const alwaysHide = this.dismissed || this.windLevelStore.alertDisabled || this.voiceStore.isMicMuted;
  ...
  getStoredDismissedStatus(): boolean {
    const dismissedDate = this.persistentStorage.getItem(WIND_NOISE_ALERT_DISMISSED_KEY);
  ...
  setUserDismissedAlertAt() {
    this.persistentStorage.setItem(WIND_NOISE_ALERT_DISMISSED_KEY, JSON.stringify(Date.now()));
  }
}
