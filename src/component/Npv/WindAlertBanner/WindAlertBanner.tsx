...
const WindAlertBanner = () => {
  const uiState = useStore().airVentInterferenceController.windAlertBannerUiState;
  ...
  return (
    <Banner show={uiState.shouldShowAlert} icon={icon} infoText={infoText}>
      <BannerButton text="How to fix" withDivider onClick={() => uiState.handleClickHowToFix()} />
      <BannerButton text="Hide" withDivider onClick={() => uiState.handleClickHide()} />
    </Banner>
  );
};
