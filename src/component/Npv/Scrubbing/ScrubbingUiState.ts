...
export default class ScrubbingUiState {
  ...
  constructor(imageStore: ImageStore, playerStore: PlayerStore, timerStore: TimerStore, npvUbiLogger: NpvUbiLogger) {
  ...
  get isScrubbingEnabled() {
    return this.playerStore.isPlayingSpotify && !this.playerStore.isDjTalking;
  }

  get colorChannels() {
    return get(this.imageStore.colors, this.playerStore.currentTrack.image_id) || [0, 0, 0];
  }
  ...
  resetScrubbingViewTimer() {
    window.clearTimeout(this.scrubbingTimeoutId);
    this.scrubbingTimeoutId = window.setTimeout(this.stopScrubbing, SCRUBBING_VIEW_TIMEOUT);
  }
