...
export const createNpvController = (
  ...
  return {
      ...
      if (npvStore.scrubbingUiState.isScrubbing) {
        ...
        if (viewStore.viewUnderCurrentView !== undefined) {
          if (playerStore.isPlayingSpotify) {
            ubiLogger.npvUbiLogger.logBackClicked(viewStore.viewUnderCurrentView);
          } else {
            ubiLogger.otherMediaUbiLogger.logBackClicked(viewStore.viewUnderCurrentView);
          }
        }
    ...
    handleDialPress(): void {
      if (npvStore.scrubbingUiState.isScrubbing) {
        ...
        if (playerStore.playing) {
          if (playerStore.isPlayingSpotify) {
            ubiLogger.npvUbiLogger.logDialPauseClicked(playerStore.currentTrackUri);
          } else {
            ubiLogger.otherMediaUbiLogger.logDialPauseClicked(playerStore.currentTrackUri);
          }
          playerStore.pause();
        } else {
          if (playerStore.isPlayingSpotify) {
            ubiLogger.npvUbiLogger.logDialPlayClicked(playerStore.currentTrackUri);
          } else {
            ubiLogger.otherMediaUbiLogger.logDialPlayClicked(playerStore.currentTrackUri);
          }
          playerStore.play();
        }
      }
    ...
    handleDialLongPress(): void {
      if (playerStore.isPlayingSpotify && !overlayController.anyOverlayIsShowing) {
