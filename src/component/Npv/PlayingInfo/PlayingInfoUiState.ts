...
import { ADVERTISEMENT, getShelfItemTitle, titleBasedOnType } from 'helpers/ContextTitleExtractor';
...
export default class PlayingInfoUiState {
  ...
  get isPlayingMusicAd() {
    return this.playerStore.isPlayingAd && !(this.playerStore.isPodcast || this.playerStore.isPodcastContext);
  }
  ...
  get subtitle() {
    if (this.isDjTalking) {
      return '';
    } else if (this.isPlayingMusicAd) {
      return ADVERTISEMENT;
    }
    return this.currentItem.artist_name || this.playerStore.currentTrackArtistName;
  }
  ...
  handleArtworkClick = () => {
    this.npvUbiLogger.logArtworkClicked(this.playerStore.playing, this.playerStore.currentTrackUri);

    this.playerStore.togglePlayPause();
  };

  handleArtistClick = () => {
    if (!this.playerStore.currentTrack.artists || this.playerStore.isDjTalking) {
      return;
    }
    ...
    this.tracklistStore.tracklistUiState.initializeTracklist({
      uri: artist.uri,
      title: this.remoteConfigStore.graphQLShelfEnabled ? artist.name : getShelfItemTitle(artist.name, artist.uri),
    });
