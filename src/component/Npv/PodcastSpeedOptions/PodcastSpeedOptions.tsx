...
const PodcastSpeedOptions = () => {
  ...
  return (
    <Overlay appear={FROM.BOTTOM} show={uiState.currentOverlayIsPodcastSpeed} classname={styles.podcastSpeedOverlay}>
      <div
        ...
        <Swiper
          ...
          onSlideChange={action(swiperInstance => {
            ...
            if (swiperInstance.activeIndex !== uiState.selectedItemIndex) {
              swiperInstance.slideTo(uiState.selectedItemIndex, transitionDurationMs);
            }
          })}
        ...
        >
          {uiState.speedOptions.map((item, index) => (
            <SwiperSlide key={item} virtualIndex={index}>
              {({ isActive }) => <PodcastSpeedItem speed={item} isActive={isActive} />}
            </SwiperSlide>
          ))}
        </Swiper>
      </div>
