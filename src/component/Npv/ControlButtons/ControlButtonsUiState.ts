...
export type ControlButtonSet = 'music' | 'podcast' | 'other_media' | 'music_ad' | 'free' | 'dj' | 'dj_talking';
...
export default class ControlButtonsUiState {
  ...
  get showMusicAdControls() {
    return this.isPlayingAd && !(this.playerStore.isPodcast || this.playerStore.isPodcastContext);
  }
  ...
  get showPodcastControls() {
    return this.playerStore.isPodcast || (this.isPlayingAd && this.playerStore.isPodcastContext);
  }
  ...
  get showDjControls() {
    return this.playerStore.isDj && !this.playerStore.isDjTalking;
  }

  get showDjTalkingControls() {
    return this.playerStore.isDjTalking;
  }
  ...
  get controlButtonSet(): ControlButtonSet {
    ...
    if (this.showDjControls) {
      return 'dj';
    }
    if (this.showDjTalkingControls) {
      return 'dj_talking';
    }
  ...
  handlePlayClick = () => {
    if (this.playerStore.isPlayingSpotify) {
      ...
      this.otherMediaUbiLogger.logPlayButtonClicked(this.playerStore.currentTrackUri);
    }
  ...
  handlePauseClick = () => {
    if (this.playerStore.isPlayingSpotify) {
      ...
      this.otherMediaUbiLogger.logPauseButtonClicked(this.playerStore.currentTrackUri);
    }
