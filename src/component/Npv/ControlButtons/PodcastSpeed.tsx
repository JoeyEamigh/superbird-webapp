...
const PodcastSpeed = () => {
  ...
  return (
    <ControlButton id={NpvIcon.PODCAST_SPEED} onClick={uiState.handlePodcastSpeedClick}>
      <div data-testid={`speed-icon-${uiState.podcastSpeed}`}>{getSpeedIcon(uiState.podcastSpeed)}</div>
    </ControlButton>
  );
};
