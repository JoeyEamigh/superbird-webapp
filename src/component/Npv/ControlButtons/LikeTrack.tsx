...
const LikeTrack = () => {
  ...
  return (
    <>
      {uiState.isSaved ? (
        <ControlButton id={NpvIcon.UNLIKE} onClick={uiState.handleUnlikeClick} isDisabled={!playerStore.canUnlike}>
          <IconHeartActive48 />
        </ControlButton>
      ) : (
        <ControlButton id={NpvIcon.LIKE} onClick={uiState.handleLikeClick} isDisabled={!playerStore.canLike}>
          <IconHeart48 />
        </ControlButton>
      )}
    </>
