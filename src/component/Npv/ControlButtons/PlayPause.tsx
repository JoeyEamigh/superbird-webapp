...
const PlayPause = () => {
  ...
  return (
    <>
      {uiState.isPlaying ? (
        <ControlButton id={NpvIcon.PAUSE} onClick={uiState.handlePauseClick} isDisabled={!playerStore.canPause}>
          <div data-testid="pause-icon">
            <IconPause48 />
          </div>
        </ControlButton>
      ) : (
        <ControlButton id={NpvIcon.PLAY} onClick={uiState.handlePlayClick} isDisabled={!playerStore.canPlay}>
