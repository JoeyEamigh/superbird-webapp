...
import { NoInteractionModalOption, OnboardingStepId } from 'store/OnboardingStore';
...
const NoInteractionModal = () => {
  ...
  useEffect(() => {
    ubiLogger.onboardingUbiLogger.logNoInteractionModalShown(OnboardingStepId[OnboardingStepId.LEARN_TACTILE]);
  }, [ubiLogger.onboardingUbiLogger]);

  const continueSelected = onboardingStore.noInteractionModal?.currentOption === NoInteractionModalOption.CONTINUE;
  ...
  return (
    <div className={styles.noInteraction}>
      <ButtonGroup style={{ width: '335px' }}>
        <Button
          type={continueSelected ? ButtonType.BUTTON_PRIMARY : ButtonType.BUTTON_SECONDARY}
          onClick={continueOnboarding}
        >
          Continue Tour
        </Button>
        <Button
          type={continueSelected ? ButtonType.BUTTON_SECONDARY : ButtonType.BUTTON_PRIMARY}
