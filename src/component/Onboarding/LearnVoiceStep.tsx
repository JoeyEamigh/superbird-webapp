...
const LearnVoiceStep = ({ learnVoiceData: { header, title, voiceEnabled, tts1, tts2 } }: Props) => {
  ...
  const voiceActive = voiceEnabled && onboardingStore.wakewordTriggered && !voiceStore.error;
  const transcript =
    voiceEnabled && !voiceStore.error && voiceStore.state.asr.isFinal && voiceStore.state.asr.transcript;
