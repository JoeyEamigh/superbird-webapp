...
import ShelfStore, { ItemType, ShelfItem, VOICE_IDENTIFIER } from 'store/ShelfStore';
...
export default class ShelfSwiperItemUiState {
  ...
  getcategoryItemTitle(itemCategory: string): string | undefined {
    return this.shelfStore.shelfController.getCategoryFromId(itemCategory)?.title.toLowerCase();
  }

  isHidden(item: ShelfItem): boolean {
    const index = this.shelfStore.shelfController.getItemIndex(item, this.shelfStore.shelfItems);
    return (
  ...
  isLeftItem(item: ShelfItem): boolean {
    if (this.shelfStore.shelfController.selectedItemIndex !== undefined) {
      return (
        this.shelfStore.shelfController.getItemIndex(item, this.shelfStore.shelfItems) <
  ...
  isTextPlaceholder(item: ShelfItem): boolean {
    if (item.category) {
      return (
        (item.category !== VOICE_IDENTIFIER && item.type === ItemType.UNPLAYABLE_ITEM) ||
  ...
  isContextItem(item: ShelfItem): boolean {
    return item.type === ItemType.PLAYABLE_ITEM || this.isValidVoiceContextItem(item);
  }
  ...
  isVoiceTextPlaceholder(item: ShelfItem): boolean {
    if (this.remoteConfigStore.pushToTalkShelfEnabled) {
      return item.category === VOICE_IDENTIFIER && item.type === ItemType.SPACER && item.identifier !== 'spacer-2';
    }
    return false;
  }
  ...
  isValidVoiceContextItem(item: ShelfItem): boolean {
    if (item.category === VOICE_IDENTIFIER) {
      if (this.remoteConfigStore.pushToTalkShelfEnabled) {
        return ![ItemType.MORE, ItemType.UNPLAYABLE_ITEM, ItemType.SPACER].includes(item.type);
      }
      return item.type !== ItemType.MORE;
    }
    return false;
  ...
  artworkClicked(item: ShelfItem): void {
    ...
    if (item.uri) {
      interactionId = this.contentShelfUbiLogger.logShelfItemClicked(
        item.category,
        this.shelfStore.shelfController.getIndexOfItemInCategory(item.uri, item.category),
        item.uri,
      );
    }
