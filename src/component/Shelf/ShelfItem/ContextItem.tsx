  ...
  return (
    ...
    >
      <LazyImage
        ...
        isActive={isActive}
      />
      <div className={styles.titleContainer} data-testid={isActive ? 'selected-item-title' : ''}>
        <div className={styles.title}>{uiState.graphQlEnabled ? title : getShelfItemTitle(title, uri)}</div>
      </div>
      <div className={styles.subtitle} data-testid={isActive ? 'selected-item-subtitle' : ''}>
        {uiState.showNowPlaying(uri) ? <NowPlaying playing={uiState.isPlaying} textName="balladBook" /> : subtitle}
      </div>
    </div>
  );
};
