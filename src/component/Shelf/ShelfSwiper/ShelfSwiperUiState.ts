...
export const getAdjustedIndex = (
  ...
  const isSelectableItem = allShelfItems[draggedToIndex]?.type !== ItemType.SPACER;

  const isVoiceSpacer =
    allShelfItems[draggedToIndex]?.type === ItemType.SPACER &&
    allShelfItems[draggedToIndex]?.category === VOICE_IDENTIFIER;

  if (isSelectableItem || (draggedToIndex === selectedItemIndex && !isVoiceSpacer)) {
  ...
  if (isVoiceSpacer) {
    const lastVoiceSpacerIndex = allShelfItems[draggedToIndex + 1]?.category !== VOICE_IDENTIFIER;
  ...
  return draggedToIndex > selectedItemIndex ? draggedToIndex + 1 : draggedToIndex - 1;
};
...
export default class ShelfSwiperUiState {
  ...
  handleDraggedToIndex(index: number): void {
    ...
    if (this.selectedItemIndex !== undefined) {
      const userDraggedToItem = this.allShelfItems[getAdjustedIndex(index, this.allShelfItems, this.selectedItemIndex)];
