...
const SWIPER_OFFSET_AFTER = SWIPER_WIDTH - ARTWORK_WIDTH - SWIPER_OFFSET_BEFORE + 2; // 2px for swiper end not to break

const THREE_ARTWORK_WIDTH = 3 * ARTWORK_WIDTH + 2 * SWIPER_SPACE_BETWEEN;
const SWIPER_SLIDES_PER_VIEW = 3 + (SWIPER_WIDTH - THREE_ARTWORK_WIDTH) / (ARTWORK_WIDTH + SWIPER_SPACE_BETWEEN); // Magical number to fit everything with correct widths and margins
...
const maybeCorrectSlide = (swiperRef, uiState, dragging) => {
  runInAction(() => {
    if (!dragging && uiState.selectedItemIndex !== swiperRef.current?.activeIndex) {
  ...
  return (
    ...
    >
      {uiState.allShelfItems.map((item: ShelfItem, index: number) => {
        return (
          <SwiperSlide key={getKey(item)} virtualIndex={index} onClick={() => uiState.dismissBanner()}>
