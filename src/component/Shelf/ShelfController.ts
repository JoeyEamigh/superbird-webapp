...
class ShelfController {
  ...
  constructor(
    ...
    this.swiperUiState = new ShelfSwiperUiState(this, this.playerStore, this.viewStore, this.contentShelfUbiLogger);
    ...
    this.voiceMuteBannerUiState = createVoiceMuteBannerUiState(this.voiceStore, this.contentShelfUbiLogger);

    this.onCategoryChange((newCategoryId, prevCategoryId) => this.handleCategoryChanged(newCategoryId, prevCategoryId));
  }
  ...
  getItemIndex(item: ShelfItem, shelfItems: ShelfItem[]): number {
    return shelfItems.findIndex(
      shelfItem => item.identifier === shelfItem.identifier && item.category === shelfItem.category,
    );
  }

  getIndexOfItemInCategory(uri: string | undefined, category: string): number {
    const categoryData = this.shelfCategories.find(c => c.parsedId === category);
    return categoryData ? categoryData.items.findIndex(it => it.uri === uri) : -1;
  }
  ...
  get rightItem(): ShelfItem | undefined {
    if (this.selectedItem && this.selectedItemIndex !== undefined) {
      return (
        this.shelfItems.slice(this.selectedItemIndex + 1).find(item => item.type !== ItemType.SPACER) ??
        this.selectedItem
      );
    }
    return undefined;
  ...
  get firstLibraryCategoryId(): string | undefined {
    return this.shelfCategories.find(category => !MAIN_CATEGORY_IDS.includes(category.parsedId))?.parsedId;
  }
  ...
  get currentShelfCategoryVisibleItems(): number | undefined {
    const activeCategory = this.shelfCategories.find(category => category.parsedId === this.selectedItem?.category);
    return activeCategory && activeCategory.itemsVisible;
  }

  getCategoryFromId(categoryId): Category | undefined {
    return this.shelfCategories.find(category => category.parsedId === categoryId);
  }

  getAllShelfItemsFromCategories = (shelfCategories: Category[]): ShelfItem[] => {
  ...
  getNewSelectedItem(
    ...
    const oldSelectedItemIndex = oldSelectedItem ? this.getItemIndex(oldSelectedItem, oldShelfItems) : undefined;
    ...
    if (oldSelectedItem && newShelfItems.some(item => item.identifier === oldSelectedItem?.identifier)) {
      return oldSelectedItem;
    }
    ...
    const newItemsInCategory = newShelfItems.filter(item => item.category === oldSelectedItem?.category);

    const oldFirstIndexInCategory = oldShelfItems.findIndex(item => item.category === oldSelectedItem?.category);
    const oldItemCategoryIndex =
      oldSelectedItemIndex !== undefined ? oldSelectedItemIndex - oldFirstIndexInCategory : undefined;

    const oldCategoryItems = oldShelfItems.filter(item => item.category === oldSelectedItem?.category);
    ...
    const matchingItemToRight = oldCategoryItems
      ...
      .find(oldItem => newItemsInCategory.find(newItem => newItem.identifier === oldItem.identifier));
    ...
    const newFirstIndexInCategory = newShelfItems.findIndex(item => item.category === oldSelectedItem?.category);
  ...
  populateShelfState(categoriesFromDataStores: Category[]) {
    const previousVisibleByActiveCategory = this.currentShelfCategoryVisibleItems;
    const activeCategory = this.shelfCategories.find(category => category.parsedId === this.selectedItem?.category);
    const previousInitialVisibleNumber = this.getShelfInitialVisibleNumber(activeCategory);
    const allNewShelfItems = this.getAllShelfItemsFromCategories(categoriesFromDataStores);

    const newSelectedItem = this.getNewSelectedItem(this.selectedItem, this.shelfItems, allNewShelfItems);
    this.shelfStore.setShelfCategories(categoriesFromDataStores);
    this.updateSelectedItem(newSelectedItem);

    if (previousVisibleByActiveCategory && previousVisibleByActiveCategory > previousInitialVisibleNumber) {
      this.openCategory(this.selectedItem?.category, previousVisibleByActiveCategory);
    }
  ...
  handlePushToTalkSelected(item: ShelfItem) {
    if (item.type === ItemType.UNPLAYABLE_ITEM && item.category === VOICE_IDENTIFIER) {
  ...
  handleItemSelected(selectedItem: ShelfItem | undefined, interactionId?: string): void {
    if (!selectedItem) {
      return;
    }
    ...
    } else if (selectedItem.type === ItemType.PLAYABLE_ITEM && selectedItem.uri) {
      window.setTimeout(() => this.updateSelectedItem(selectedItem), transitionDurationMs);
  ...
  private handleContextItemSelected(item: ShelfItem): void {
    if (item.category === VOICE_IDENTIFIER) {
      setTrackViewVoiceFeatureIdentifier(item, this.voiceStore, this.shelfStore);
    }
    this.tracklistStore.tracklistUiState.handleSupportedTracklists({
      uri: item.uri,
      title: this.remoteConfigStore.graphQLShelfEnabled ? item.title : getShelfItemTitle(item.title, item.uri),
      image_id: item.image_id,
    });
    if (
      isSupportedUriType(item.uri) &&
      !this.tracklistStore.tracklistUiState.isStationAndNotCurrentlyPlaying(item.uri)
    ) {
  ...
  private handleTrackOrEpisodeSelected(item: ShelfItem, interactionId?: string) {
    ...
    if (item.uri === this.playerStore.currentTrackUri) {
      ...
      const featureIdentifier = getShelfFeatureIdentifierForTrackOrEpisode(item, this.shelfStore, this.voiceStore);
  ...
  updateToFirstItemOfCategory(categoryId: string): void {
    const firstItemOfType = this.shelfItems.find(item => item.category === categoryId);
    if (firstItemOfType) {
      this.updateSelectedItem(firstItemOfType);
    }
  }

  openCategory(itemCategory: string | undefined, numberOfItemsToShow = FIRST_PAGE_SIZE): void {
    ...
    categoryToExpand.itemsVisible = Math.min(numberOfItemsToShow, categoryToExpand.items.length);
  ...
  expandCategory(itemCategory: string): void {
    ...
    if (categoryToExpand.items.length <= this.getShelfInitialVisibleNumber(categoryToExpand)) {
      return;
    }
  ...
  collapseNotActiveCategories(id: string): void {
    const categoriesToCollapse = this.shelfCategories.filter(category => category.parsedId !== id);
  ...
  handleCategoryChanged(newCategoryId: string | undefined, prevCategoryId: string | undefined) {
    if (newCategoryId && prevCategoryId) {
      ...
      const previousCategoryExpanded = prevCategory.itemsVisible > INITIAL_VISIBLE_ITEMS;

      if (previousCategoryExpanded) {
        this.collapseTimeoutIds[prevCategoryId] = window.setTimeout(() => {
          runInAction(() => {
            this.swiperUiState.animateSliding = false;
            prevCategory.itemsVisible = this.getShelfInitialVisibleNumber(prevCategory);
          });
        }, TIME_BEFORE_COLLAPSING_CATEGORY);
      }
    }
  ...
  handleDialPress() {
    ...
    let interactionId;
    if (this.selectedItem && this.selectedItem.type === ItemType.PLAYABLE_ITEM) {
    ...
    } else if (this.selectedItem && this.selectedItem.type === ItemType.MORE && this.selectedItemCategory) {
      interactionId = this.contentShelfUbiLogger.logMoreButtonDialPressed(this.selectedItemCategory);
    } else if (
      this.selectedItem &&
      ...
      this.shelfSwiperItemUiState.pushToTalkDialPressed(this.selectedItem.category);
    }
   ...
   * Reactions
   */
  onCategoryChange(callback: (newCategoryId: string | undefined, prevCategoryId: string | undefined) => void) {
    reaction(
