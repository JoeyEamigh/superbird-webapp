...
import ShelfStore, { Category, MAIN_CATEGORY_IDS, YOUR_LIBRARY } from 'store/ShelfStore';
...
export default class ShelfHeaderUiState {
  ...
  get isInYourLibrary(): boolean {
    return !MAIN_CATEGORY_IDS.includes(this.shelfStore.shelfController.selectedItemCategory);
  }

  get mainCategories(): Array<Category> {
    return this.shelfStore.shelfCategories.filter(category => MAIN_CATEGORY_IDS.includes(category.parsedId));
  }
  ...
  get activeTitleIndex(): number {
    ...
    return titleCategories.findIndex(categoryId => this.shelfStore.shelfController.selectedItemCategory === categoryId);
  }
  ...
  headerItemClicked = (id: string): void => {
    const clickCategoryId = id === YOUR_LIBRARY ? this.shelfStore.shelfController.firstLibraryCategoryId : id;
