...
class Jellyfish extends Component<Props, State> {
  ...
  maybeUpdateState(t: number): void {
    if (this.state.animationState === animationStates.LISTENING && this.thinking) {
      this.setState({
    ...
    } else if (this.state.animationState === animationStates.THINKING && this.error) {
      this.setState({ animationState: animationStates.THINKING_TO_ERROR });
      this.transitionStart = t;
      this.startTransitionTimeout(animationStates.ERROR);
    } else if (this.state.animationState === animationStates.LISTENING && this.error) {
      this.setState({ animationState: animationStates.LISTENING_TO_ERROR });
      this.transitionStart = t;
      this.startTransitionTimeout(animationStates.ERROR);
    } else if (this.state.animationState === animationStates.ERROR && !this.error) {
  ...
  getListeningRadius({ wave, degree }: RenderArgs): number {
    ...
    const adjustedAmplitude = wave.listening.amplitude * (1 + voiceStore.micLevelMovingAverage * 2);

    return this.warpedRadius(degree, wave.listening.radius, wave.listening.frequency, adjustedPhase, adjustedAmplitude);
  }

  warpedRadius(degree: number, radius: number, frequency: number, phase: number, amplitude: number): number {
    const x = (degree - 100) / radius;
    return radius - this.getActivation(x) * (amplitude + amplitude * Math.sin(x * frequency - phase));
  }
  ...
  getRadius(args: RenderArgs) {
    ...
    switch (this.state.animationState) {
      case animationStates.LISTENING_TO_THINKING:
        return this.transition(this.getListeningRadius(args), this.getThinkingRadius(args), t);
      ...
      case animationStates.THINKING_TO_ERROR:
        return this.transition(this.getThinkingRadius(args), wave.error.radius, t);
      case animationStates.ERROR:
        return wave.error.radius;
      case animationStates.LISTENING_TO_ERROR:
        return this.transition(this.getListeningRadius(args), wave.error.radius, t);
      case animationStates.ERROR_TO_LISTENING:
        return this.transition(wave.error.radius, this.getListeningRadius(args), t);
      default:
  ...
  getLineWidth({ wave, t }: Omit<RenderArgs, 'degree'>): number {
    switch (this.state.animationState) {
      case animationStates.LISTENING_TO_THINKING:
        return this.transition(wave.listening.lineWidth, wave.thinking.lineWidth, t);
      case animationStates.LISTENING:
        return wave.listening.lineWidth;
      case animationStates.THINKING:
        return wave.thinking.lineWidth;
      case animationStates.THINKING_TO_ERROR:
        return this.transition(wave.thinking.lineWidth, wave.error.lineWidth, t);
      case animationStates.ERROR:
        return wave.error.lineWidth;
      case animationStates.LISTENING_TO_ERROR:
        return this.transition(wave.listening.lineWidth, wave.error.lineWidth, t);
      case animationStates.ERROR_TO_LISTENING:
        return this.transition(wave.error.lineWidth, wave.listening.lineWidth, t);
      default:
  ...
  getColor({ wave, t }: Omit<RenderArgs, 'degree'>): string {
    const getIntermediateColor = (c1: string, c2: string) => {
      ...
      const c1Parsed: Array<number> = (c1.match(rgbaExtractor) || []).map(parseFloat);
      const c2Parsed: Array<number> = (c2.match(rgbaExtractor) || []).map(parseFloat);
      const currentColors = c1Parsed.map((channel1, index) => this.transition(channel1, c2Parsed[index], t));
  ...
  startAnimation() {
    ...
    const renderCanvas = (timestamp: number) => {
      ...
      this.phase =
        (this.phase + Math.PI * this.listeningRotationSpeed + voiceStore.micLevelMovingAverage * 0.5) % (2 * Math.PI);
