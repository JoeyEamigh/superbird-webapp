...
const Equaliser = ({ playing }: Props) => {
  const [eqAnimation, setEqAnimation] = useState(playing ? EqAnimation.PLAYING : EqAnimation.PAUSED);
  ...
  useEffect(() => {
    ...
    const startPausingTimer = () => {
      ...
      pauseTimeout.current = window.setTimeout(() => setEqAnimation(EqAnimation.PAUSED), PAUSE_PLAY_TRANSITION_MS);
    };
