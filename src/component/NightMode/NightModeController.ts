...
import createNightModeUiState, { NightModeUiState } from 'component/NightMode/NightModeUiState';
...
class NightModeController {
  ...
  constructor(rootStore: RootStore) {
    this.nightModeUiState = createNightModeUiState(rootStore.hardwareStore, rootStore.remoteConfigStore);
      ...
      (current, prev) => {
        [
          ...
          if ((prev < min || prev > max) && current >= min && current <= max) {
            rootStore.ubiLogger.nightModeUbiLogger.logNightModeImpression(logString);
          }
        });
      },
    );
  ...
  get isNightMode() {
    return Boolean(JSON.parse(this.storage.getItem(NIGHT_MODE_USER_ENABLED_KEY) ?? 'false'));
  }

  toggleNightMode(): void {
    this.storage.setItem(NIGHT_MODE_USER_ENABLED_KEY, JSON.stringify(!this.isNightMode));
  }
}
