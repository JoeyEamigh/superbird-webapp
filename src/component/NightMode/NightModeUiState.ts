...
const roundToTwoDecimals = (n: number) => Math.round((n + Number.EPSILON) * 100) / 100;

const nightModeUiStateFactory = (hardwareStore: HardwareStore, remoteConfigStore: RemoteConfigStore) => {
  return makeAutoObservable({
    get appOpacity(): number {
      return roundToTwoDecimals(
        1 - (this.nightModeSlope * hardwareStore.ambientLightValue + remoteConfigStore.nightModeStrength - 100) / 100,
      );
    },
