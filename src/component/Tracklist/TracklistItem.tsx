...
const TracklistItem = ({ item, isActive = false }: Props) => {
  ...
  const isCurrent =
    ...
    (canCompareUid ? item.uid === uiState.currentPlayingTrackUid : item.uri === uiState.currentPlayingTrackUri);
    ...
    >
      {showImage && (
        <div className={styles.imageContainer}>
          <div className={styles.image} data-testid={`track-image-${item.uri}`}>
            <LazyImage uri={item.uri} size={96} scale={1.3} imageId={item.image_id} isActive={isActive} />
          </div>
        </div>
      )}
      <div
      ...
      >
        ...
        <div className={styles.subtitleContainer}>
          {item.available_offline && <IconDownloadAltActive className={styles.downloaded} />}
          {item.isTrailer && <Trailer />}
          {metadata?.is_explicit_content && (
            <img src="images/explicit.svg" alt="explicit" className={styles.explicit} />
          )}
          <Type name="balladBook" className={styles.subtitle} dataTestId={`${isActive ? 'track-subtitle' : ''}`}>
            {item.subtitle}
          </Type>
          {isPodcast && metadata?.is_played && <IconCheckAltActive className={styles.playedIcon} />}
