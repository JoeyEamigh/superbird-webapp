...
import { isCollectionUri, isPlaylistRecommendedUri, isTrackOrEpisode, isDjUri } from 'helpers/SpotifyUriUtil';
import {
  isPlaylistV1OrV2URI,
  isSearchURI,
  isShowURI,
...
export const isSupportedUriType = (uri: string): boolean =>
  !(isTrackOrEpisode(uri) || isSearchURI(uri) || isPlaylistRecommendedUri(uri) || isDjUri(uri));
...
class TracklistUiState {
  ...
  get shouldShowLatestPlayedEpisode(): boolean {
    return this.isPodcastContext && this.rootStore.podcastStore.shouldShowLatestPlayedEpisode(this.contextUri);
  }

  get latestPlayedEpisode(): TrackItem | undefined {
    return this.isPodcastContext
      ? this.tracksList.find(
          trackItem => trackItem.uri === this.rootStore.podcastStore.getLatestPlayedUri(this.contextUri),
        )
      : undefined;
  }
  ...
  get initiallySelectedItem(): TrackItem {
    if (this.isNowPlayingContext) {
      ...
      this.rootStore.errorHandler.logUnexpectedError(
        new Error('Should scroll to the latest played podcast episode, but it was not in the list of episodes'),
      );
    }
  ...
  async loadInitialItems(uri: string): Promise<void> {
    ...
    const { childItemStore, podcastStore, onboardingStore, viewStore } = this.rootStore;
    ...
    return (
      (
        isShowURI(this.contextUri)
          ? this.rootStore.podcastStore.getPodcastItemsIncludingTrailer(this.contextUri)
  ...
  get currentlyPlayingItemSelected(): boolean {
    const currentlyPlayingTrackIndex = this.tracksList.findIndex(
      (item: TrackItem) => item.uri === this.rootStore.playerStore.currentTrack.uri,
    );
    return currentlyPlayingTrackIndex >= 0 && currentlyPlayingTrackIndex === this.selectedItemIndex;
  }
  ...
  get currentTrackInTracklist(): boolean {
    return !!this.tracksList.find((item: TrackItem) => item.uri === this.rootStore.playerStore.currentTrack.uri);
  }

  isStationAndNotCurrentlyPlaying(contextUri: string): boolean {
    return isStationURI(contextUri) && contextUri !== this.rootStore.playerStore.contextUri;
  }
  ...
  loadCurrentContext(): void {
    ...
    if (items.length > 0) {
      ...
      if (firstItem.image_id && isShowURI(playerStore.contextUri) && firstItem.uri === playerStore.contextUri) {
  ...
  handleSupportedTracklists(contextItem: ContextItem) {
    ...
    } else if (isDjUri(contextItem.uri)) {
      this.rootStore.playerStore.summonDj(HOME_IDENTIFIER);
      viewStore.showNpv();
  ...
  get isPodcastOrAlbum(): boolean {
    return this.contextType === URITypeMap.ALBUM || this.contextType === URITypeMap.SHOW;
  }

  get rightItem(): TrackItem | undefined {
    if (this.selectedItemIndex === this.tracksList.length - 1 || this.selectedItemIndex < 0) {
  ...
  getIndexOfItem(trackItem?: TrackItem): number {
    if (isStationURI(this.contextUri)) {
      const indexById = this.tracksList.findIndex(item => trackItem?.id === item.id);
  ...
  loadMore(): void {
    ...
    if (this.isPodcastContext) {
      this.rootStore.podcastStore.loadMore(this.contextUri, PAGE_SIZE, this.selectedItemIndex, MIN_DISTANCE_FROM_END);
    } else {
      this.rootStore.childItemStore.loadMore({
        parentId: this.contextUri,
        pageSize: PAGE_SIZE,
        currentIndex: this.selectedItemIndex,
        numberOfItemsLoaded: this.rootStore.childItemStore.getNumberOfItems(this.contextUri),
        totalNumberOfItemsAvailable: this.rootStore.childItemStore.getTotal(this.contextUri),
  ...
  handleDraggedToIndex(index: number): void {
    this.setShouldShowAddToQueueBanner(false);
    const userDraggedToItem = this.tracksList.find((_, itemIndex) => index === itemIndex);
  ...
  handleItemSelected(item: TrackItem, interactionId: string) {
    ...
    const isTrackInContext =
      ...
      (canCompareUid ? item.uid === playerStore.currentTrack.uid : item.uri === playerStore.currentTrack.uri);
    ...
    viewStore.showNpv();
    window.setTimeout(() => this.updateSelectedItem(item), transitionDurationMs);
  }
  ...
  get currentlyPlayingTrackOrFirst(): TrackItem {
    const currentlyPlayingTrackUri = this.rootStore.playerStore.currentTrack.uri;
    const currentlyPlayingItem = this.tracksList.find((item: TrackItem) => item.uri === currentlyPlayingTrackUri);
  ...
  handleTrackChange() {
    ...
    } else if (isStationURI(this.contextUri) && this.selectedItemIndex === -1 && this.browsingCurrentContext) {
      const newItem = this.tracksList.find(item => this.selectedItem?.uri === item.uri);
      if (newItem) {
  ...
  logRemoveLike() {
    this.rootStore.ubiLogger.trackListUbiLogger.logContextRemoveLike(this.contextUri);
  }

  logLike() {
    this.rootStore.ubiLogger.trackListUbiLogger.logContextLiked(this.contextUri);
  }
