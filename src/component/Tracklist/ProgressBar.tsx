...
const ProgressBar = (props: PlayingBarProps) => {
  ...
  if (isCurrent) {
    return (
      <div data-testid={`progress-bar-${uri}`} className={styles.progressBar}>
        <div className={styles.progress} style={{ width: `${props.store.timerStore.trackPlayedPercent}%` }} />
      </div>
    );
  }
  ...
  if (metadata) {
    ...
    if (time_left_ms && duration_ms && showPausedProgressBar) {
      return (
        <div data-testid={`progress-bar-${uri}`} className={styles.progressBar}>
          <div className={styles.progress} style={{ width: `${100 - (time_left_ms / duration_ms) * 100}%` }} />
        </div>
      );
    }
  }
