...
class PhoneCallUiState {
  ...
  constructor(phoneCallController: PhoneCallController, phoneCallUbiLogger: PhoneCallUbiLogger) {
  ...
  get shouldShowAnswerOrDeclineButtons(): boolean {
    return !this.isOutgoing && (this.store.isRingingIncoming || !this.hasAnswered);
  }
  ...
  get title(): string {
    return this.store.callerName || this.store.callerNumber || NUMBER_UNAVAILABLE;
  }
