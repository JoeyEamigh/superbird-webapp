...
class PhoneCallController {
  ...
  constructor(rootStore: RootStore, socket: Socket, middlewareActions: MiddlewareActions) {
    this.androidPhoneCallStore = new AndroidPhoneCallStore(rootStore, rootStore.interappActions, socket);
    this.iphonePhoneCallStore = new IphonePhoneCallStore(rootStore, middlewareActions, socket);
    ...
    this.phoneCallUiState = new PhoneCallUiState(this, rootStore.ubiLogger.phoneCallUbiLogger);

    makeObservable(this, { store: computed, isUserEnabled: computed });

    this.iphonePhoneCallStore.onCallInitiated(() => rootStore.overlayController.maybeShowAModal());
    this.iphonePhoneCallStore.onCallEnd(() => rootStore.overlayController.resetAndMaybeShowAModal());
    this.androidPhoneCallStore.onCallInitiated(() => {
      rootStore.overlayController.maybeShowAModal();
    });
    this.androidPhoneCallStore.onCallEnd(() => rootStore.overlayController.resetAndMaybeShowAModal());
  }

  get isUserEnabled(): boolean {
    return Boolean(JSON.parse(this.persistentStorage.getItem(PHONE_CALL_USER_ENABLED_KEY) ?? 'true'));
  }

  get store(): PhoneCallStore {
    return this.remoteControlStore.phoneType === 'iOS' ? this.iphonePhoneCallStore : this.androidPhoneCallStore;
  }

  get shouldShowPhoneCallOverlay(): boolean {
    return (
      this.isUserEnabled &&
      (this.store.isRingingIncoming || this.store.isRingingOutgoing || this.store.isOngoingPhoneCall)
    );
  }
  ...
  toggleIsUserEnabled(): void {
    this.persistentStorage.setItem(PHONE_CALL_USER_ENABLED_KEY, JSON.stringify(!this.isUserEnabled));
  }
