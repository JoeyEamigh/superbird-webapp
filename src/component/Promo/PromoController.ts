...
class PromoController {
  ...
  constructor(
    { remoteConfigStore, settingsStore, voiceStore, viewStore, overlayController, persistentStorage }: RootStore,
    middlewareActions: MiddlewareActions,
  ) {
  ...
  get promoHistory(): PromoSeenHistory {
    const storedValue = this.persistentStorage.getItem(PROMO_HISTORY_LOCAL_STORAGE_KEY);
  ...
  get nextPromoToShow(): Promo | undefined {
    ...
    return PROMOS_ORDERED.find(promo => this.isEnabled(promo) && !this.hasSeenPromo(promo));
  }

  get shouldShowPromo(): boolean {
    return !this.userHasSeenPromoThisSession && this.nextPromoToShow !== undefined && this.viewStore.isMain;
  }

  handlePromoSeen(promo: Promo): void {
    const new_history = this.promoHistory;
    new_history[promo] = true;
    this.persistentStorage.setItem(PROMO_HISTORY_LOCAL_STORAGE_KEY, JSON.stringify(new_history));
