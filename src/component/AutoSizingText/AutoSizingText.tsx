...
const AutoSizingText = ({
  ...
  useEffect(() => {
    const effect = () => {
      ...
      if (preRenderDiv && preRenderDiv.offsetHeight > maxHeight && textSizeIndex + 1 < textSizesDescending.length) {
    ...
    runInAction(effect);
  }, [ref, refText, textContent, textSizeIndex, showText, maxHeight, textSizesDescending]);

  return showText ? (
    <Type name={textSizesDescending[textSizeIndex]} className={className} dataTestId={dataTestId} ref={ref}>
      {textContent}
    </Type>
  ) : null;
};
