...
const PresetContent = ({ preset }: Props) => {
  ...
  return (
    <>
      <LazyImage imageId={preset.image_url} size={168} uri={preset.context_uri} isActive={isFocused} />
      <div className={styles.presetTitle}>
        ...
        >
          {isRadioStationURI(preset.context_uri) ? `Radio · ${preset.name}` : preset.name}
        </Type>
