...
export const isPresetButtonsEnabled = (rootStore: RootStore): boolean => {
  const { viewStore, permissionsStore, remoteControlStore, sessionStateStore, overlayController } = rootStore;
...
class PresetsController {
  ...
  get isSwipeDownPresetsEnabled() {
    return this.isPresetButtonsEnabled && !this.rootStore.overlayController.isShowing('voice');
  }
}
