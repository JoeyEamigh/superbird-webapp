import PresetsDataStore, { Preset, PRESET_NUMBERS, PresetNumber } from 'store/PresetsDataStore';
...
export type PresetOrPlaceholderOrUnavailable = PresetItem | PlaceholderItem | UnavailableItem;
...
class PresetsUiState {
  ...
  getPresetFromCurrentContext(withNumber: PresetNumber): Preset {
    const imageId =
      this.shelfStore.shelfItems.find(item => item.uri === this.currentContextUri)?.image_id ??
      this.playerStore.currentImageId;
  ...
  highlightPreset(): void {
    const indexOfNowPlayingInPresets = this.presets.findIndex(
      preset => preset.type === 'preset' && this.playerStore.contextUri === preset.context_uri,
    );
  ...
  playPresetOrTts(presetNumber: PresetNumber, interactionId?: string): void {
    ...
    if (this.presetsDataStore.isUnavailable(presetNumber)) {
      ...
      this.playerStore.playItem(presetUri, FEATURE_IDENTIFIER_PRESETS, interactionId);
    } else {
  ...
  handleTapOnPreset(presetNumber: PresetNumber): void {
    ...
    this.updateSelectedPreset(presetNumber);
    window.setTimeout(() => this.overlayController.resetAndMaybeShowAModal(), PRESET_SELECT_TO_PLAY_TIMEOUT);
  }
  ...
  private logPresetClicked = (presetNumber: PresetNumber): string | undefined => {
  ...
  logDialTurn = () =>
    this.presetsUbiLogger.logDialTurn(
      this.selectedPresetNumber,
      this.getTempOrSavedPreset(this.selectedPresetNumber)?.context_uri ?? 'placeholder-uri',
    );
  ...
  logTouchPlaceHolder = (presetNumber: PresetNumber) => this.presetsUbiLogger.logTouchPlaceHolder(presetNumber);
}
