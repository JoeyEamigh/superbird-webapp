...
import { isRadioStationURI, isTrackOrEpisode, isYourEpisodesUri } from 'helpers/SpotifyUriUtil';
...
const LazyImage = ({
  ...
  useEffect(() => {
    if (shouldLoad && imageId && size && !ImageStore.isLocalFileImage(imageId)) {
  ...
  const getImageTemplate = (image: string) => {
    ...
    const innerBorderColor = showInnerBorder ? getInnerBorderColor() : undefined;
  ...
  if (!imageId || ImageStore.isLocalFileImage(imageId)) {
    return wrapComponent(<Placeholder uri={uri} size={size} onClick={onClick} scale={scale} isActive={outerBorder} />);
  }
